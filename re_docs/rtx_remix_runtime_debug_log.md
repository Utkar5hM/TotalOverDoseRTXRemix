...
## Alpha Effect Depth Refinement - 2026-06-02 22:46 +05:30

User report:
- Continue work on invisible destructible objects.
- Sun rays/glow appear to pass through objects.
- Reference screenshots:
  `C:\GOG Games\Total Overdose\screenshots\sun`.

Interpretation:
- The previous alpha-effect A8 gate made sparks/blood visible, but also allowed
  depth-disabled glow/flare sprites. Those are authored as overlays and can pass
  through world geometry if treated as visible effect textures.
- Invisible destructible objects are likely a different alpha path: alpha-tested
  cutout/world draws with depth testing and depth writes enabled. They can block
  movement and have destruction effects even if the base A8 textured draw is
  skipped by Remix.

Change:
- Refined `IsAlphaEffectA8Draw()` into two render-state cases:
  - depth-tested particle: depth enabled, depth writes disabled, alpha blend or
    alpha test enabled;
  - alpha-tested cutout/world object: depth enabled, depth writes enabled,
    alpha test enabled, alpha blend disabled.
- Depth-disabled alpha overlays are now excluded. This should suppress sun/glow
  overlay leakage while keeping sparks/blood and allowing alpha-tested
  destructible/cutout geometry to get hashable DEFAULT-pool A8 copies.
- No content hashes, config parsing, dirty refresh, candidate dumping, or texture
  dimension expansion.

Build result:
- Live ASI: `scripts\TODCameraResend.asi`
- Size: `120832` bytes
- Timestamp: `2026-06-02 22:46:17`
- Backup:
  - `tools\TODCameraResend.alpha_depth_refined_20260602.asi`
  - `tools\tod_camera_resend_asi\TODCameraResend.alpha_depth_refined_20260602.cpp.bak`
- Rollback to previous sparks/blood build:
  `tools\TODCameraResend.alpha_effect_a8_20260602.asi`.
- Rollback to verified state4 snapshot:
  `C:\GOG Games\TotalOverDoseRTXRemix\state4`.

Expected test:
- RTX remains visible.
- HUD remains visible.
- Sparks/blood remain visible.
- Sun/glow overlay should no longer visibly pass through depth-disabled cases.
- Some invisible destructible/cutout objects may become visible if they use
  alpha-test with normal depth state.

## Managed A8 Depth-World Draw Test - 2026-06-02 23:12 +05:30

User report:
- Asked whether the render-state approach is appropriate.
- Sun appears fixed after the depth refinement.
- The blocking/destructible object is still not visible.
- Video:
  `C:\Users\utkar\Videos\NVIDIA\Total Overdose\Total Overdose 2026.06.02 - 22.57.24.08.mp4`.

Evidence:
- Extracted review frames to:
  `re_docs\invisible_object_225724_frames`.
- Contact sheet:
  `re_docs\invisible_object_225724_frames\contact_1fps.jpg`.
- Full-resolution representative frame:
  `re_docs\invisible_object_225724_frames\t04_0_full.jpg`.
- The object appears to be a small/thin destructible prop or barrier near the
  corrugated wall. It is not a sun/depth overlay.
- Prior missing-texture diagnostic already showed many skipped primary-target
  fixed-function managed `A8R8G8B8` world draws with real geometry
  (`FVF 0x112`, `FVF 0x152`, and declaration-path draws). Those are exactly the
  class a small destructible A8 prop would fall into if its texture lacks a
  Remix-valid hash.

Appropriateness:
- The render-state approach is appropriate when the rule describes a real D3D
  draw category, not a mission/object/texture-name exception.
- It would become inappropriate if we kept adding one-off hashes, texture sizes,
  or location-specific cases.
- This test stays generic: fixed-function managed A8 world draws on the primary
  target with depth enabled are allowed; depth-disabled overlays remain excluded.

Change:
- Broadened the previous `IsAlphaEffectA8Draw()` into
  `IsManagedA8DrawCopyCandidate()`.
- The managed A8 draw-copy gate now permits three depth-state categories:
  - depth-tested particles: depth enabled, depth writes disabled;
  - alpha-tested cutout/world objects: depth enabled, depth writes enabled,
    alpha test enabled, alpha blend disabled;
  - opaque A8 world albedo/props: depth enabled, depth writes enabled,
    alpha blend disabled, alpha test disabled.
- Still excludes:
  - RHW/POSITIONT screen-space draws handled by the HUD path;
  - vertex-shader draws;
  - offscreen render-target draws;
  - depth-disabled sun/lens/glow overlays;
  - video/cutscene RT paths.
- No content hashes, no config parsing, no dirty refresh, no candidate dumps, no
  texture-size whitelist expansion.

Build result:
- Live ASI: `scripts\TODCameraResend.asi`
- Size: `120832` bytes
- Timestamp: `2026-06-02 23:12:17`
- Backup:
  - `tools\TODCameraResend.managed_a8_depth_world_20260602.asi`
  - `tools\tod_camera_resend_asi\TODCameraResend.managed_a8_depth_world_20260602.cpp.bak`
- Rollback to previous sun-fixed test:
  `tools\TODCameraResend.alpha_depth_refined_20260602.asi`.
- Rollback to verified state4 snapshot:
  `C:\GOG Games\TotalOverDoseRTXRemix\state4`.
...
- RTX remains visible.
- HUD remains visible.
- Sun/glow remains fixed.
- Sparks/blood remain visible.
- The destructible blocking prop may become visible.
- If broad world material corruption returns, reject the opaque A8 category and
  do a focused metadata/dump diagnostic at this exact object instead of widening
  further.

## Managed A8 Depth-Blend Cutout Test - 2026-06-02 23:23 +05:30

User evidence:
- User provided an original non-RTX screenshot of the same area.
- The missing object is a large breakable corrugated/wood wall panel covering
  the opening on the left wall, not a tiny prop or pole.
- User also noted the sun/glow effect can still appear through objects
  sometimes, so depth-disabled overlays must remain excluded.

Appropriateness:
- This is still the appropriate kind of ASI rule because it describes a real
  D3D9 draw category: managed `A8R8G8B8` fixed-function world draws on the
  primary target with depth enabled.
- It does not identify this panel by hash, dimensions, mission, screen
  position, or texture contents.
- If this category creates broad material corruption, the next step should be a
  focused capture around the panel's draw state, not adding content exceptions.

Change:
- Extended `IsManagedA8DrawCopyCandidate()` to also permit depth-writing
  alpha-blended world cutouts:
  `depthEnabled && depthWriteEnabled && alphaBlendEnabled`.
- This covers old D3D content that uses alpha blending rather than alpha test
  for breakable/cutout panels.
- Depth-disabled overlays remain excluded, so full-screen sun/lens/glow sprites
  are still not copied by this path.
- No content hashes, no config parser, no DDS dump path, no runtime logging, no
  size whitelist expansion.

Build result:
- Live ASI: `scripts\TODCameraResend.asi`
- Size: `120832` bytes
- Timestamp: `2026-06-02 23:23:00`
- Backup:
  - `tools\TODCameraResend.managed_a8_depth_blend_20260602.asi`
  - `tools\tod_camera_resend_asi\TODCameraResend.managed_a8_depth_blend_20260602.cpp.bak`
- Rollback to previous depth-world test:
  `tools\TODCameraResend.managed_a8_depth_world_20260602.asi`.
- Rollback to verified state4 snapshot:
  `C:\GOG Games\TotalOverDoseRTXRemix\state4`.
...
- HUD remains visible.
- Sparks/blood remain visible.
- The breakable wall/panel becomes visible before it is destroyed.
- Sun/glow should not be worse. If it is still visible through objects, that is
  likely a separate depth-tested glow/particle classification problem rather
  than this panel path.

## Managed A8 Cutouts Without Opaque World - 2026-06-02 23:48 +05:30

User result:
- The missing breakable wall/panel texture is now visible.
- New/regressed issue: mission 1/2 sky/background can become blue, but the sky
  and large world surfaces glitch. Screenshots saved in:
  `screenshots\sky`.
- Mission 3 sky does not show the same blue result.

Screenshot read:
- `Total Overdose Screenshot 2026.06.02 - 23.38.56.18.png` shows a large pale
  blue/white world surface and strong sun/glow bleeding.
- `Total Overdose Screenshot 2026.06.02 - 23.40.48.82.png` shows a blue sky,
  but with a large terrain/backdrop surface intersecting/glitching across it.
- This looks like two overlapping problems:
  - Remix sky autodetection (`rtx.skyAutoDetect = 2`) may now classify some sky
    in missions 1/2;
  - the ASI's managed A8 copy rule was still too broad for normal opaque world
    materials, which can corrupt terrain/sky/backdrop surfaces.

Change:
- Removed the opaque-world category from `IsManagedA8DrawCopyCandidate()`:
  `depthEnabled && depthWriteEnabled && !alphaBlendEnabled && !alphaTestEnabled`.
- Kept only categories that carry explicit effect/cutout semantics:
  - depth-tested particles: `depthEnabled && !depthWriteEnabled`;
  - alpha-tested cutouts: `depthEnabled && depthWriteEnabled &&
    alphaTestEnabled && !alphaBlendEnabled`;
  - alpha-blended depth-writing cutouts: `depthEnabled && depthWriteEnabled &&
    alphaBlendEnabled`.
- This preserves the clean render-state approach while avoiding the broadest
  albedo path that previous notes already flagged as risky for sky/world
  material corruption.
- No config change yet. `rtx.skyAutoDetect = 2` remains in both config paths
  until the ASI-side world-material regression is ruled in/out.

Build result:
- Live ASI: `scripts\TODCameraResend.asi`
- Size: `120832` bytes
- Timestamp: `2026-06-02 23:48:29`
- Backup:
  - `tools\TODCameraResend.managed_a8_cutouts_no_opaque_20260602.asi`
  - `tools\tod_camera_resend_asi\TODCameraResend.managed_a8_cutouts_no_opaque_20260602.cpp.bak`
- Rollback to panel-visible but broad opaque-world test:
  `tools\TODCameraResend.managed_a8_depth_blend_20260602.asi`.

Expected test:
- The breakable panel should ideally remain visible.
- Pale/blue terrain or floor corruption should improve if the opaque A8 category
  was the cause.
- HUD and sparks/blood should remain visible.
- If sky is still blue but backdrop geometry still intersects/glitches, then the
  next clean target is explicit sky/backdrop classification in Remix config, not
  another broad A8 shadow rule.

## TOD Camera Farclip Override Test - 2026-06-03 00:31 +05:30

Context:
- User reported current state has RTX active, HUD visible, sparks/blood fixed,
  and the previously invisible breakable panel visible.
- Remaining issue: Remix capture/render distance feels too short; capture cannot
  include the whole visible scene.
- Ghidra RE shows TOD has a real camera `farclip` property at `camera + 0xBC`,
  registered from `FUN_0087e1e0`, with current-camera storage through
  `DAT_00a3dcbc + 0x6c`.

Change:
- Added a guarded engine-camera farclip patch to
  `tools\tod_camera_resend_asi\TODCameraResend.cpp`.
- The ASI resolves `TOD.exe` module base + RVA `0x0063DCBC` to read
  `DAT_00a3dcbc`.
- It checks camera-system slots `+0x6c`, `+0x60`, and `+0x64`.
- It validates memory with `VirtualQuery`, checks sane `nearclip/farclip`
  values, and writes only `camera + 0xBC = 3000.0f` when the current farclip is
  below the test value.
- The patch runs once per `Present`, so it follows camera swaps without touching
  draw calls.
...
- Saved test ASI: `tools\TODCameraResend.camera_farclip_3000_20260603.asi`
- Saved test source:
  `tools\tod_camera_resend_asi\TODCameraResend.camera_farclip_3000_20260603.cpp.bak`
- Rollback ASI: `tools\TODCameraResend.pre_farclip_20260603.asi`
- Rollback source:
  `tools\tod_camera_resend_asi\TODCameraResend.pre_farclip_20260603.cpp.bak`
- State4 remains the saved working baseline and was not overwritten by this
  test build.

Expected test:
- RTX, HUD, sparks/blood, and the breakable panel should remain as in state4.
- Remix capture/render distance may improve after the first rendered frame.
- If far geometry appears but flicker/z precision worsens, lower the override
  to `2000.0f` before considering any culling bypass.

## SkyBox Geometry Config Test - 2026-06-03 01:10 +05:30

User result:
- With the `3000.0f` camera farclip build, more world geometry appears to be
  captured/rendered by Remix.
- Next target is sky/backdrop handling.

Analysis:
- Sky should remain a Remix classification/config problem, not an ASI texture
  copy rule.
- Ghidra confirms a real TOD `SkyBox` class registration:
  `FUN_008f22a0()` registers `"SkyBox"`.
- A prior sky capture identified three mesh hashes using a solid sky-like
  material/texture `86CB3E6CC84F9371`:
  - `mesh_08AB62B56C25BD67`
  - `mesh_0B600FFAE9C09AD0`
  - `mesh_734739A160FD48E6`
- Geometry tagging is safer than `rtx.skyBoxTextures` because TOD reuses many
  terrain/backdrop-looking textures in normal world geometry.

Change:
- Added this line to both `scripts\rtx.conf` and root `rtx.conf`:

```ini
rtx.skyBoxGeometries = 0x08AB62B56C25BD67, 0x0B600FFAE9C09AD0, 0x734739A160FD48E6
```

- Left `rtx.skyAutoDetect = 2` enabled for now. This pass only adds confirmed
  manual sky geometry; it does not remove autodetection.
...
Expected test:
- If the skybox hashes are active in the tested mission, sky should become more
  stable/visible without changing HUD, particles, panel textures, or farclip.
- If sky is still black or distant terrain still cuts across the sky, the next
  pass needs a fresh capture at the exact current sky problem angle and should
  inspect mesh/material hashes from that capture, not broaden ASI A8 rules.

## SkyBox Texture + Early Drawcall Test - 2026-06-03 01:28 +05:30

User result:
- Mission 2 sky remained black after the three `rtx.skyBoxGeometries` hashes.
- Other missions not checked yet.

Checks:
- Latest `bridge32.log` did not report `rtx.skyBoxGeometries` as a parse
  failure.
- Installed `.trex\d3d9.dll` contains these local option strings:
  - `rtx.skyBoxTextures`
  - `rtx.skyDrawcallIdThreshold`
  - `rtx.skyForceHDR`
  - `rtx.skyReprojectToMainCameraSpace`
- No fresh Remix capture was created after the mission-2 black-sky test, so
  live mission-2 sky hashes are still unknown.

Change:
- Added confirmed solid-sky texture hash from the older sky capture:

```ini
rtx.skyBoxTextures = 0x86CB3E6CC84F9371
```

- Added conservative early-draw sky classification:

```ini
rtx.skyDrawcallIdThreshold = 12
```

- Applied to both `scripts\rtx.conf` and root `rtx.conf`.
- Left `rtx.skyAutoDetect = 2` and existing `rtx.skyBoxGeometries` in place.
...
Expected test:
- Relaunch and check mission 2 sky.
- If sky appears, reduce/validate the drawcall threshold later so we do not tag
  normal early world draws.
- If sky remains black, capture a Remix frame at the exact black-sky mission-2
  angle; without a fresh capture, the live mission-2 sky mesh/texture cannot be
  identified cleanly.

## Clean Sky Auto-Detect Test - 2026-06-03

User result from the previous sky pass:

- Missions 1 and 2 still had black sky.
- Mission 3 had visible sky, but the backdrop/sky geometry looked distorted
  and blocky.

Reasoning:

- The manual `rtx.skyBoxGeometries` list came from an older capture:

```ini
rtx.skyBoxGeometries = 0x08AB62B56C25BD67, 0x0B600FFAE9C09AD0, 0x734739A160FD48E6
```

- The latest captured frame contains different sky-ish mesh names
  (`mesh_B67B874D7BCDD65D`, `mesh_97B9E3F5DBA2BD1B`, etc.), so keeping the old
  geometry list is stale and can misclassify unrelated distant geometry in some
  missions.
- Local Remix option strings explain that `rtx.skyAutoDetect = 2` only treats
  the first camera as sky if that draw has depth testing disabled. Ghidra shows
  TOD's sky flows through the normal render-list / mesh path, so the sky can be
  depth-tested and fail mode-2 detection.
- Ghidra also confirms a real TOD `SkyBox` class (`FUN_008f22a0`) and generated
  sky mesh construction (`FUN_008f20b0`), but the render submission appears to
  flow through generic model/render-list fields rather than a simple dedicated
  D3D sky draw hook.

Change:

- Removed stale `rtx.skyBoxGeometries` from both config files.
- Changed sky auto-detection from mode 2 to mode 1 in both config files:

```ini
rtx.skyAutoDetect = 1
```

- Did not add `rtx.skyBoxTextures`, `rtx.skyDrawcallIdThreshold`, or any ASI
  sky logic in this pass.

Backups:

```text
tools\scripts_rtx.pre_sky_autodetect1_clean_20260603.conf.bak
tools\rtx.pre_sky_autodetect1_clean_20260603.conf.bak
```

Expected test:

- Relaunch required; Remix reads config at startup.
- Check missions 1 and 2 for black sky.
- Check mission 3 for whether the blocky/distorted backdrop is reduced after
  removing stale manual sky geometry.

## Mission-2 Orange Sky Probe Analysis - 2026-06-03

User result after `rtx.skyAutoDetect = 1`:

- Mission 1 sky filled in.
- Missions 2 and 3 no longer stayed black, but the sky became a huge orange /
  yellow flare.
- User captured mission 2 as:

```text
scripts\rtx-remix\captures\capture_2026-06-03_02-04-52.usd
```
...
```text
re_docs\capture_2026-06-03_020452_skyprobe_half_preview.png
re_docs\capture_2026-06-03_020452_thumb_half_preview.png
re_docs\capture_2026-06-03_020452_texture_sheet.png
```

- The captured sky probe is almost entirely black with one saturated yellow
  blob, matching the orange/yellow sky seen in-game.
- The capture contains one explicit sky instance:

```text
sky_3EC6766F06C8CB82
```

- The corresponding mesh is bound to:

```text
mesh_3EC6766F06C8CB82 -> mat_5EF9EBC260F4B6BC
```

- Texture `5EF9EBC260F4B6BC` is a 64x64 yellow sun/lens-flare sprite, not a
  skybox or distant backdrop texture.

Conclusion:

- `rtx.skyAutoDetect = 1` is too broad for TOD in missions 2/3. It lets the
  sun/lens-flare sprite pollute the sky probe.
- This is still a Remix classification/config issue. No ASI sky logic is
  justified by this result.

Change:

- Added a narrow native Remix ignore category for only the confirmed flare
  texture in both `rtx.conf` files:
...
```text
tools\scripts_rtx.pre_ignore_sunflare_20260603.conf.bak
tools\rtx.pre_ignore_sunflare_20260603.conf.bak
```

Expected test:

- Relaunch required.
- Mission 1 should ideally keep its filled sky.
- Mission 2/3 should no longer use the yellow flare as the sky probe. If they
  fall back to black sky, the next clean pass should disable broad auto-detect
  and identify the real mission sky/backdrop meshes/textures from captures.

## Explicit Sky Geometry Pass - 2026-06-03

User result after ignoring flare texture `5EF9EBC260F4B6BC`:

- Sky became black on all three tested missions.
- User captured all three missions:

```text
scripts\rtx-remix\captures\capture_2026-06-03_02-26-21.usd
scripts\rtx-remix\captures\capture_2026-06-03_02-27-22.usd
scripts\rtx-remix\captures\capture_2026-06-03_02-29-04.usd
```

Capture outputs generated:

```text
re_docs\capture_20260603_022621_thumb_half_preview.png
re_docs\capture_20260603_022621_texture_sheet.png
re_docs\capture_20260603_022722_thumb_half_preview.png
re_docs\capture_20260603_022722_texture_sheet.png
re_docs\capture_20260603_022904_thumb_half_preview.png
re_docs\capture_20260603_022904_texture_sheet.png
re_docs\sky_like_texture_candidates_20260603_0229.png
```

Findings:

- The black-sky captures do not contain any `sky_*` instances and did not emit
  a `T_SkyProbe.dds`. This confirms Remix had no accepted sky after the flare
  ignore.
- The mission-3/city capture still includes the previously observed dark teal
...
- Meshes confirmed bound to that material across captures:
...
- `rtx.skyAutoDetect = 1` is too broad and can select flare geometry.
- Ignoring the flare proves broad autodetect had no other accepted sky in these
  scenes.
- The cleaner next pass is to disable autodetect and explicitly tag only
  confirmed sky-material geometry using Remix's native `rtx.skyBoxGeometries`.
...
- Removed `rtx.ignoreTextures = 0x5EF9EBC260F4B6BC`; with autodetect disabled
  the flare should not be promoted to sky, and hiding/ignoring the flare is not
  necessary.
- No ASI change.
...
```text
tools\scripts_rtx.pre_explicit_sky_geometries_20260603.conf.bak
tools\rtx.pre_explicit_sky_geometries_20260603.conf.bak
```

Expected test:

- Relaunch required.
- Mission 3/city should be the most likely to improve because its current
  capture includes `mesh_B67B874D7BCDD65D -> mat_86CB3E6CC84F9371`.
- Missions 1/2 may still need their own explicit sky geometry if the tested
  captures were aimed away from the sky or use different sky assets.

## Engine-Based SkyBox Viewport Marker - 2026-06-03

User result from the explicit-geometry config: black/sun-only in mission 1 and
colored ray artifacts in mission 3. This means the fixed sky geometry list is
not a reliable global solution.

Ghidra RE found the proper TOD sky path:

```text
FUN_008F22A0                 registers class "SkyBox"
FUN_008F2360                 SkyBox constructor, sets render-object flag 0x08000000
TOD_SkyBox_UpdateOrSubmit_008F1E10
  owns/reuses render list at SkyBox + 0x7C
  emits five static sky meshes from DAT_00A3E0B0[0..4]
  calls FUN_00432C70 for each mesh, producing render-list opcode 0x11
FUN_004342C0                 render-list interpreter
FUN_004540E0                 opcode 0x11 indexed mesh draw
```

The engine's stable sky mesh buffer mapping is:

```text
mesh + 0x0C -> VB wrapper pointer, flags in low 2 bits
mesh + 0x10 -> IB wrapper pointer, flags in low 2 bits
...
- Added `kMarkTodSkyDrawsWithViewportMinZ`.
- On indexed draws, query current stream-0 vertex buffer and index buffer.
- Compare them against `DAT_00A3E0B0[0..4]`.
- If the draw uses a TOD SkyBox mesh, temporarily set viewport
  `MinZ=0.999, MaxZ=1.0`, call `DrawIndexedPrimitive`, then restore the
  original viewport.
- Runtime logging remains compiled out (`TOD_ENABLE_RUNTIME_LOG 0`).

Config change in both `rtx.conf` and `scripts\rtx.conf`:
...
Removed the fixed `rtx.skyBoxGeometries` pass from active configs. This should
let Remix detect TOD's actual SkyBox draws globally through a D3D state cue,
without per-mission sky hashes.
...
scripts\TODCameraResend.asi size: 123392 bytes, timestamp 2026-06-03 03:11:43
```

Backups:

```text
tools\TODCameraResend.pre_sky_viewport_marker_20260603.cpp.bak
tools\rtx.pre_sky_viewport_marker_20260603.conf.bak
tools\scripts_rtx.pre_sky_viewport_marker_20260603.conf.bak
```

## SkyBox Texture Discovery Pass - 2026-06-03

User result from the engine viewport marker: mission 1/2 showed only a blue
patch in the upper-left with the rest of the sky black, and mission 3 did not
show a stable sky during gameplay. This confirms that matching
`DAT_00A3E0B0[0..4]` does identify real SkyBox draws, but the `MinZ` viewport
cue is not enough to make Remix classify TOD's SkyBox correctly.

Archive string scan found the real TOD sky asset families in `blocks.naz`:
...
The same scan found the known false-positive flare/effect assets:
...
- `kMarkTodSkyDrawsWithViewportMinZ = false` so the partial blue-patch marker is
  disabled.
- `kEnableTodSkyTextureDiscovery = true` enables a narrow SkyBox-only capture.
- On each indexed draw, the ASI still matches the current VB/IB against
  `DAT_00A3E0B0[0..4]`.
- For matched draws only, it hashes/dumps the original stage-0 source texture
  bound by TOD, not the DEFAULT replacement texture created for Remix.
- Outputs:
...
Active config for the diagnostic:
...
This intentionally disables Remix's unstable sky autodetect while collecting
the exact SkyBox runtime texture hashes. The intended final fix is to add the
confirmed SkyBox hashes to Remix sky config, then rebuild with
`kEnableTodSkyTextureDiscovery = false`. No sky texture hash should live in the
ASI.
...
cmd /c tools\tod_camera_resend_asi\build.bat
Built scripts\TODCameraResend.asi
scripts\TODCameraResend.asi size: 131584 bytes, timestamp 2026-06-03 11:23:37
```

Follow-up correction:

- User reported the whole game became black before useful capture.
- No `rtx-remix\logs\tod-sky-textures.tsv` was produced.
- Restored `rtx.skyAutoDetect = 2` in both configs so Remix returns to the
  previous partially navigable sky path while the SkyBox-only capture remains
  enabled.
- Moved SkyBox texture hashing/dumping out of `DrawIndexedPrimitive`: the draw
  hook now only queues the original stage-0 SkyBox source texture, and the
  actual lock/hash/DDS write runs at `Present`.
...
cmd /c tools\tod_camera_resend_asi\build.bat
Built scripts\TODCameraResend.asi
scripts\TODCameraResend.asi size: 132096 bytes, timestamp 2026-06-03 11:59:26
active sky config: rtx.skyAutoDetect = 2, rtx.skyMinZThreshold = 0.99
```

## 2026-06-03 - Forced TextureMap Probe Added

User asked whether the ASI can force the sky textures through the render path so
manual mission-by-mission capture is not required.

Implemented a compile-time-gated diagnostic probe using the engine's own
texture debug helper rather than ASI-authored fake sky draws:
...
At `Present`, the ASI calls `Texture::DrawAllTextures()` for five frames. While
that function is active, stage-0 `SetTexture` calls are queued, then hashed and
DDS-dumped at `Present`.
...
This should expose every texture already present in TOD's runtime texture map
without requiring the user to visit every mission. It will not load unloaded
mission archive assets by itself; if sky variants are absent from the probe,
the next clean step is to RE the engine asset-loader path for the skybox
families instead of adding mission-specific ASI content cases.
...
- `tod-forced-texture-probe.tsv` produced 23 rows.
- `re_docs\forced_texture_probe_contact_sheet.png` confirms the captured set is
  UI/HUD/flags/icons/video/render targets, not skybox textures.
- No `tod-sky-textures.tsv` was produced because no real SkyBox draw happened
  during this menu/boot run.

## 2026-06-03 - Forced Sky Texture Asset Loader Probe

Ghidra-backed correction to the force-through-ASI approach:

- `Texture::DrawAllTextures()` is valid, but it only renders textures already in
  TOD's runtime `Texture::TexturesMap`.
- To avoid playing every mission, the ASI must first load the SkyBox BMP assets
  through TOD's own native asset loader.

Ghidra decompile evidence:

```text
AssetLoader::LoadAssetByName @ 0x008FFC10
  calls LoadNativeResource if FindLoadedAsset misses
  then blindly calls IncreaseResourceReferenceCount(asset)
  => unsafe ASI entry point for optional diagnostics

Loadscreen texture path @ 0x0087C2F0
  sets DAT_00A3BE18 = 6
  sets DAT_00A3BE2D = 1
  calls LoadNativeResource @ 0x00878AB0
...
The ASI now loads the known SkyBox BMP resource paths found in `blocks.naz`,
logs success/null/exception rows to:

```text
rtx-remix\logs\tod-sky-asset-load-probe.tsv
```

Then the existing forced texture-map pass hashes/dumps any loaded textures to:

```text
rtx-remix\logs\tod-forced-texture-probe.tsv
rtx-remix\logs\tod-forced-texture-probe\*.dds
```
...
- User reported startup crash.
- `tod-sky-asset-load-probe.tsv` shows the probe successfully loaded the first
  13 paths on each attempted launch.
- The process died before row 14, which would have been:

```text
/data/Textures/Skybox/Sky/cloudSky/skybox_RT.bmp
```

- No new forced texture rows were emitted after the asset-load rows, so the
  crash happened before the follow-up `Texture::DrawAllTextures()` pass could
  hash/dump the newly loaded assets.

Conclusion: direct `LoadNativeResource` from `Present` is not a safe generic
sky preload mechanism even though individual assets return non-null. It mutates
TOD's resource manager state in a way that can crash startup, likely because
these assets are being loaded outside the block/scene ownership context that
normally owns SkyBox resources.
...
Next clean direction: do not force arbitrary sky assets through
`LoadNativeResource` at startup. Either capture the real SkyBox draw when a
mission naturally loads it, or hook the existing SkyBox/material binding path
after TOD has selected a valid sky family for the current scene.

## 2026-06-03 - SkyBox Capture Fallback Added

After rollback, user confirmed startup no longer crashes.

Ghidra follow-up:

```text
SkyBox::Render @ 0x008F1E10
  calls FUN_00856EC0(0, faceIndex, 0) with ECX = SkyBox->ModelRes
  returns material pointer in EAX
  emits FUN_00431660(renderList, material, 0)
  emits FUN_00432C70(DAT_00A3E0B0[face])

Render-list opcode 1
  FUN_0044EF70(material, stage)
  stores material at DAT_00A39F14 + 0x96F8 + stage * 4

Texture flush
  FUN_0044F8A0 -> FUN_004634B0(material, stage)
  calls IDirect3DDevice9::SetTexture(stage, *(material + 4) & ~3)
```

ASI change:

- Keep startup asset forcing disabled.
- Keep broad texture-map probe disabled.
- For verified SkyBox mesh draws only, hash the original source texture if
  registered; otherwise hash the bound DEFAULT copy.
- Fallback rows are marked `ok_bound_fallback` in
  `rtx-remix\logs\tod-sky-textures.tsv`.
...
## 2026-06-03 - SkyBox::Render Scoped Texture Capture

User ran the game again after the draw-capture fallback. No
`rtx-remix\logs\tod-sky-textures.tsv` was produced and
`rtx-remix\logs\tod-sky-textures` stayed empty, so the VB/IB matcher did not
see the sky path in that run.

Added a narrower engine-scope diagnostic hook:

```text
SkyBox::Render @ 0x008F1E10
first instruction: 81 EC 90 00 00 00  ; SUB ESP, 0x90
patch size: 6 bytes
patch opcode: PUSH Hook_TodSkyBoxRender ; RET
trampoline: original 6 bytes, then PUSH target+6 ; RET
```

The hook does not render, force-load, or change TOD state. It only increments a
thread-local-style atomic depth counter while the original `SkyBox::Render`
runs. The normal `IDirect3DDevice9::SetTexture` hook now queues stage-0 texture
binds while that depth is non-zero.
...
Rows with `skyDraw=0` came from scoped `SkyBox::Render` material binds. Rows
with `skyDraw>0` came from the older verified VB/IB draw matcher.

Because general runtime logging is disabled in this build, the same TSV also
gets one-shot status rows:

```text
scope_hook_installed
scope_entered
scope_hook_unavailable / scope_hook_unexpected_prologue / scope_hook_install_failed
```

This makes a no-texture run interpretable without re-enabling the large
per-frame log.
...
## 2026-06-03 - SkyBox Material Command Capture

The first scoped run produced:
...
but no texture rows. That proves `SkyBox::Render` runs, but TOD does not call
D3D `SetTexture` inside that function. It builds a render list and flushes the
material later.

Added a second scoped diagnostic hook:

```text
FUN_00431660 @ 0x00431660
  render-list opcode 1 writer
  thiscall: ECX = render list, args = (material, stage)
  validated prologue: 56 57 8B F9 8B 47 20
  patch size: 7 bytes
```

While `SkyBox::Render` depth is non-zero, this hook captures stage-0 material
commands and extracts the D3D texture pointer from `material + 4`, matching the
already reversed material flush path:

```text
FUN_004634B0(material, stage)
  SetTexture(stage, *(material + 4) & ~3)
```

This still does not force-load sky assets or emit extra draws; it records the
actual sky materials TOD selected for the active scene.

Expected additional TSV statuses:

```text
material_hook_installed
material_hook_unavailable / material_hook_unexpected_prologue / material_hook_install_failed
scope_material_no_texture
```
...
## 2026-06-03 - Captured SkyBox Material Textures Applied

User ran the material-command capture build. Result:
...
The dumped contact sheet is:

```text
re_docs\skybox_material_contact_sheet_20260603.png
```

Captured SkyBox material hashes:

```text
0x3848EB42A9454B29  64x64   A8R8G8B8
0x38F01A3A44A105E4  128x128 DXT1
0x3B3C8BFC46BDDD15  128x128 DXT1
0x3CBFF5A558504B9F  64x64   A8R8G8B8
0x988E3EC036BC1681  64x64   A8R8G8B8
0x98BC8203A806B55D  128x128 DXT1
0xBFD77BC11E21AE42  64x64   A8R8G8B8
0xDDF1BE54CD1936C8  128x128 DXT1
0xDF12A2DCB829B60D  128x128 DXT1
0xE4F08883065FBB37  64x64   A8R8G8B8
```

Config change:

- Added all 10 confirmed sky material hashes to `rtx.skyBoxTextures`.
- Set `rtx.skyAutoDetect = 0` so Remix no longer guesses sky by camera/depth
  heuristics now that the real texture hashes are known.
- Removed `0xDDF1BE54CD1936C8` from `rtx.uiTextures`; it had been a false
  positive HUD/UI candidate but is now proven to be a SkyBox face.
- Made `rtx.conf` and `scripts\rtx.conf` byte-identical to avoid config drift.

Active config hashes:

```text
rtx.conf                 SHA256 9C75DA1B60A8E1BF686347CC11D241616AC219E690BCD1FE46A59CE7941F19EC
scripts\rtx.conf         SHA256 9C75DA1B60A8E1BF686347CC11D241616AC219E690BCD1FE46A59CE7941F19EC
```

## 2026-06-03 - Second Sky Family Captured From Mission 1

User reported sky now appears across missions but looks like the same blue sky
family. The latest `tod-sky-textures.tsv` confirmed the reason: the config only
contained the first captured SkyBox family, while mission 1 also emitted a
second yellow/horizon family.

Additional captured hashes:

```text
0x062000B7F573945E  128x128 A8R8G8B8
0x164B13483E1BB6C2  128x128 A8R8G8B8
0xE0A8EF0E70BA5ABC  128x128 A8R8G8B8
0xFA794A119892B086  128x128 A8R8G8B8
0xFE36AFFA25F09DE5  128x128 A8R8G8B8
0x1D1D8E3DB8CE7310  64x64   A8R8G8B8
0x4D29CD6D0157132D  64x64   A8R8G8B8
0x7919EC4E5955B32E  64x64   A8R8G8B8
0xA55EB28F0D50E1DC  64x64   A8R8G8B8
0xE6EC0E321A0D5CA9  64x64   A8R8G8B8
0x2F45551FAFF6107F  8x8     A8R8G8B8
0x6087F9EB4BEC07AE  8x8     A8R8G8B8
0x68B8DC7027BB5EA5  8x8     A8R8G8B8
0xAB767F0AC00773F9  8x8     A8R8G8B8
0xD9D0D405A049B4A1  8x8     A8R8G8B8
```
...
- Expanded `rtx.skyBoxTextures` from 10 to 25 confirmed SkyBox material hashes.
- Removed `0x1D1D8E3DB8CE7310` from `rtx.uiTextures`; it was another false
  positive UI candidate and is part of the mission-1 sky family.
- Root and scripts configs remain byte-identical:

```text
rtx.conf                 SHA256 F68B500DA6915C9B91FD93DA4A67CC1C7BE96089F826249684B48028A00EBC94
scripts\rtx.conf         SHA256 F68B500DA6915C9B91FD93DA4A67CC1C7BE96089F826249684B48028A00EBC94
```

## 2026-06-03 - Runtime SkyBox Mesh Marker For All Missions

User clarified the real goal: all skies should work properly across missions,
not just the sky families discovered so far. The static `rtx.skyBoxTextures`
list is useful evidence/fallback, but it is not the clean end state because TOD
can load more SkyBox texture families.

Ghidra path confirmed:

```text
SkyBox::Render @ 0x008F1E10
  FUN_00431660(renderList, material, stage)  ; material opcode
  FUN_00432C70(renderList, mesh)             ; mesh opcode 0x11

Render-list execution:
  opcode 0x11 -> FUN_004540E0(mesh)
  FUN_004540E0 calls IDirect3DDevice9::DrawIndexedPrimitive
```

ASI change:

- Keep the `SkyBox::Render` scope hook.
- Hook `FUN_00432C70` only to record mesh pointers added while inside
  `SkyBox::Render`.
- Hook `FUN_004540E0` to set `g_todSkyMeshDrawDepth` while those recorded
  SkyBox meshes are actually drawn.
- The D3D9 `DrawIndexedPrimitive` hook now treats that mesh execution scope as
  a confirmed TOD SkyBox draw and temporarily sets viewport `MinZ = 0.999`.
- The viewport is restored immediately after the draw.
...
Local Remix source confirms `skyMinZThreshold` is an explicit sky detection
path independent of `skyAutoDetect`; this lets the ASI mark exactly TOD's real
SkyBox mesh drawcalls without relying on camera heuristics or a complete
texture hash list.
...
cmd /c tools\tod_camera_resend_asi\build.bat
Built scripts\TODCameraResend.asi
scripts\TODCameraResend.asi size: 138752 bytes, timestamp 2026-06-03 20:42:17
```

Active config hashes:

```text
rtx.conf                 SHA256 6B753F3EA62D7D53F48846B61BCF95B43530913536C32769B0ACAA1A63383A72
scripts\rtx.conf         SHA256 6B753F3EA62D7D53F48846B61BCF95B43530913536C32769B0ACAA1A63383A72
```

### 2026-06-03 20:57 - Switched sky test to runtime mesh marker only

User still saw the blue sky after the captured mission-1 yellow/orange SkyBox
material hashes were added to `rtx.skyBoxTextures`. This means the static hash
list is not a clean final solution: it can tag known textures, but it can also
make Remix treat unrelated or earlier sky-family draws as the active sky.

Changed both `rtx.conf` files:
...
Removed `rtx.skyBoxTextures` for this A/B so the only sky classification route
is the ASI marking the actual TOD SkyBox mesh draw with viewport `MinZ = 0.999`.

Added TSV one-shot status rows:

- `sky_mesh_draw_entered`: emitted by the reversed `RenderMesh::Draw` hook when
  a mesh recorded during `SkyBox::Render` is executed.
- `sky_viewport_marked` / `sky_viewport_not_marked`: emitted by the D3D indexed
  draw hook when it sees that TOD SkyBox draw and attempts the MinZ marker.

Build:

```text
scripts\TODCameraResend.asi size: 138752 bytes, timestamp 2026-06-03 20:57:56
...
### 2026-06-03 21:10 - Blue sky after runtime marker: test Remix fog path

User still saw a blue sky after the static sky texture list was removed.

The new TSV rows prove the engine-based marker path is active:

- `sky_mesh_draw_entered`
- `sky_viewport_marked`
- active marked texture: `0x98BC8203A806B55D`, 128x128 DXT1 blue SkyBox face

Therefore the remaining mismatch is not "wrong SkyBox hash" in the ASI. The
live TOD SkyBox draw is blue; the yellow/orange mission look in the original is
most likely fixed-function fog/haze/sun grading applied over that blue base.

Local Remix source confirms:

- `rtx.enableFog` defaults true and feeds fixed-function D3D9 fog into the
  final composite.
- `SceneManager::submitDrawState()` keeps the first unreplaced non-empty fog
  state in a frame.
- `rtx.fogIgnoreSky` makes `D3D9Rtx::PrepareDraw()` clear fog state for draws
  already categorized as Sky.

Config-only A/B now active in both configs:
...
No ASI rebuild required. Config SHA256:

```text
rtx.conf / scripts\rtx.conf SHA256 0E0C52B88267ECF744BBC0EAA14109A6361E7219D1E6A795C29303F0E21DEE84
```

### 2026-06-05 - Animated alpha-world vertex-buffer freeze test

Sky/haze state accepted by user; focus moved to tree/foliage flicker and unstable
alpha structures.

Previous evidence in `tod_render_distance_texture_handling_re_20260603.md`:

- Camera resend is not the cause; disabling it made geometry worse.
- Billboard index layout was captured and remapped successfully, but the visual
  glitch did not change.
- DLFG amplifies motion artifacts but is not the root; standing-still foliage
  still flickers.
- Remix requires vertex positions in the generation hash; removing them crashes
  with `Position hash should never be empty`.

Implemented a contained ASI test:

- `kFreezeAnimatedAlphaWorldVertexBuffers = true`
- `kEnableTodFogStateLog = false`
- For large fixed-function indexed alpha world draws with depth write enabled,
  cache a compact frozen copy of the used stream-0 vertex range and bind it only
  for that draw. Restore the original stream source immediately after the draw.
- Thresholds:
  - `primitiveCount >= 80`
  - `numVertices >= 64`
  - primary render target, no vertex shader, non-RHW/non-POSITIONT

Expected effect:

- If TOD foliage sway/CPU animation is the cause, foliage becomes more static
  but stable and stops rehashing every frame in Remix.
- If the wrong things freeze, tighten the candidate rule; do not broaden.
...
```text
scripts\TODCameraResend.asi size: 142848 bytes, timestamp 2026-06-05 11:26:31
scripts\TODCameraResend.asi SHA256 1CDCF6C832BDF4DE730C86DEDD0E0E84D31BAB8FAACAB239898075A2D07560C7
TODCameraResend.cpp SHA256 11029D07C5208D6CE5818E0FCD8A7072F47928968200E634C5A438B4834ABB6F
rtx.conf / scripts\rtx.conf SHA256 8223AECE0E8E986E09A29B4CD4F1FFD056F1A34414FA8687ECDA6A899E8FF987
```

### 2026-06-05 11:50 - Rejected VB-freeze test; disabled fixed sun injection

User result with the VB-freeze build:

- Foliage/structure flicker remained.
...
```text
rtx-remix\logs\tod-foliage-freeze.tsv
created=3, bound=23, lockFails=0, createFails=0
matched draw class: primitiveCount=546, numVertices=482, stride=32
```

Interpretation:

- The freeze test matched and bound correctly, but it did not improve the flicker.
- Keep the code present but disabled.
- The black warehouse floor regions looked like hard shadow/decal artifacts, not
  missing floor geometry. The fixed ASI sun injection is a likely source because
  it applied one directional light globally, including indoors.

Active source flags now:
...
```text
scripts\TODCameraResend.asi size: 139776 bytes, timestamp 2026-06-05 11:50:31
...
- Same warehouse: if black floor regions disappear, fixed sun injection caused
  the new artifact.
- Foliage: if still flickering, proceed with targeted foliage texture/hash review
  or deeper TOD mesh-class RE; do not re-enable broad VB freeze.

### 2026-06-05 12:08 - Floor/sun test reverted; focus returns to global flicker

User result:

- Disabling the fixed sun injection did not fix the reported artifact.
- The floor/black-region issue appears limited to mission 1 at that location.
- Foliage/structure flicker remains global, so this is now the priority.

Active behavior restored:
...
- The global flicker is not fixed by the broad alpha-world VB freeze.
- Do not keep chasing the mission-1 local floor artifact as the main thread.
- Next clean test should be Remix-native and narrow: identify the unstable
  foliage/alpha world texture hashes from existing dumps and test hiding only
  confirmed problematic vegetation instances, or use TOD RE to identify a true
  vegetation mesh class before changing geometry behavior.

### 2026-06-05 12:25 - Corrected scope: broad moving glitch, not foliage-only

User clarified that the current issue is not limited to tree/foliage flicker:
many textures/objects appear to glitch while moving. Therefore do not use the
foliage `rtx.hideInstanceTextures` path for this symptom.

Action:

- Removed the temporary foliage hide test before it reached both configs.
- No `rtx.hideInstanceTextures` line is active.
- Applied one global-motion isolation test:

```ini
rtx.dlfg.enable = False
```

in both `rtx.conf` and `scripts\rtx.conf`.

Reason:

- Frame generation/interpolation can produce broad moving-object warping across
  unrelated textures and geometry.
- This is a config-only test and does not alter ASI hook behavior.

State:

```text
scripts\TODCameraResend.asi unchanged: size 139776, timestamp 2026-06-05 12:05:32
scripts\TODCameraResend.asi SHA256 9B9E2D70F0A1B0BE8B6558B5DCFBAAD2ABD27E32DDE85F9538E9252130C9EC6E
rtx.conf SHA256 1E73C571D1937168596DC6305313CDA3995D55ACDF9F58D92D1AE0EC236D2E457
scripts\rtx.conf SHA256 203B24C07341A32837C2FB6442DF1A5A44611457949A615BFA49B725F7AE8DAA
```

Next result interpretation:

- If moving glitches improve: keep DLFG off or investigate Remix motion vector /
  frame interpolation compatibility separately.
- If no change: next likely area is not texture content. Inspect frame-to-frame
  instance identity, transform timing, and render-target handoff in the ASI/TOD
  draw path.

### 2026-06-05 16:45 - Moving glitch video: previous-frame ghosting
...
- Broad previous-frame / feedback ghosting across the whole scene during zipline
  camera motion.
- The ghost includes the player silhouette and UI/text areas, so it is not a
  single tree/foliage/material hash issue.
- `rtx.postfx.enable = False` is already active, so Remix postfx motion blur is
  not the configured source.

Binary / source evidence:

- Ghidra confirms TOD has render-list commands for render-target feedback:
  `CMD_PUSH_RENDERTARGET`, `CMD_POP_RENDERTARGET`,
  `CMD_RENDERFULLSCREENTEXTURE`, `CMD_RENDERTEXTUREDQUAD*`.
- `FUN_0044e220` centralizes D3D9 `SetRenderTarget` /
  `SetDepthStencilSurface` plus viewport update.
- `FUN_004342c0` is the render-list interpreter and executes full-screen /
  textured quad commands as well as mesh commands.
- Remix source notes temporal denoising can produce ghosting/boiling/blurring
  artifacts (`rtx.useDenoiser` option comment in `rtx_options.h`).

Rejected test:

- `rtx.dlfg.enable = False` damaged sky and did not fix the moving ghosting.
- Restored:
...
added to both `rtx.conf` and `scripts\rtx.conf`.

Reason:

- DLFG controls generated frames; the artifact appears to be ordinary temporal
  history/ghosting.
- Ray Reconstruction is a narrower temporal-history test than disabling the
  whole denoiser or hiding texture instances.
...
scripts\TODCameraResend.asi unchanged: size 139776, timestamp 2026-06-05 12:05:32
scripts\TODCameraResend.asi SHA256 9B9E2D70F0A1B0BE8B6558B5DCFBAAD2ABD27E32DDE85F9538E9252130C9EC6E
rtx.conf SHA256 13B4365EDFE524149FE20B28FD3B3B0A3E1B83EB80A3968C29C63FE4F6E7652E
scripts\rtx.conf SHA256 944802452F1DFA26D53EE471416B865D2D287304AFDE13D951D23358316C43A
```

Next result interpretation:

- If ghosting improves: keep RR off; optionally tune upscaler/denoiser next.
- If unchanged: next test should be ASI-side RT routing, specifically narrowing
  `kRedirectAllColorRtTexturesToPrimary` / `kSkipAllRtTextureComposites` because
  TOD uses render-target full-screen feedback passes.

### 2026-06-05 17:02 - RR-off rejected; narrowed ASI RT routing active

User result:

- `rtx.enableRayReconstruction = False` did not fix the moving ghost/texture
  smear.

Reverted:

- Removed `rtx.enableRayReconstruction = False` from both active Remix configs.
- Kept DLFG restored:
...
- The 2026-06-05 16:34 video shows previous-frame / render-target feedback
  contamination across player, world, and UI/text, not a single texture class.
- TOD's binary has render-list commands for full-screen texture and render-target
  feedback passes.
- The previous ASI route was too broad: all color render-target textures were
  redirected to primary and all render-target texture composites were skipped.
- This test preserves the main full-size scene RT workaround, but lets offscreen
  render-target ping-pong/composite passes execute unless they are the final
  composite to the primary surface.
...
- If ghosting improves: the broad RT redirect/composite skip corrupted TOD's
  feedback chain. Keep narrowed RT routing and then regression-test HUD/sky.
- If scene goes black or RTX drops out: the full-size RT heuristic is too narrow
  or the final-composite skip is wrong; inspect exact RT dimensions during the
  zipline scene.
- If unchanged: next non-texture tests are native upscaler
  (`rtx.upscalerType = 0`) or denoiser off (`rtx.useDenoiser = False`) as
  diagnostics, plus RT-dimension logging.

### 2026-06-05 17:10 - Narrowed RT routing rejected; native/no-denoiser diagnostic active

User result:

- Narrowing ASI RT routing did not fix the moving ghost/texture smear.

Reverted:

- Restored ASI RT routing constants to the previous baseline:
...
- `rtx.enableRayReconstruction = False` alone did not change the symptom.
- The Remix source shows `upscalerType` defaults to DLSS and
  `useDenoiser` explicitly covers temporal artifacts such as ghosting, boiling,
  and blurring.
- This is a broad temporal-history isolation test. If it improves the smear,
  split the setting afterward to identify whether DLSS upscaling or denoising is
...
### 2026-06-05 17:30 - Native/no-denoiser rejected; world transform resend active

User clarification/result:

- The temporary tree/box artifact came from external texture upscaling, not the
  Remix diagnostic.
- The moving smear/glitch still exists, so the native/no-denoiser diagnostic is
  rejected.

Reverted:

- Removed these diagnostic lines from both active Remix configs:
...
- `FUN_0044def0` is TOD's world-transform submitter:

```text
SetTransform(D3DTS_WORLD = 0x100, matrix)
```

- `FUN_0044e400` submits `D3DTS_VIEW`.
- `FUN_0044e580` submits `D3DTS_PROJECTION`.
- `FUN_004540e0` flushes textures/streams/FVF/indices and issues
  `DrawIndexedPrimitive`.
- `FUN_00421530` sets view/projection for a render command list, then
  `FUN_004342c0` interprets the draw command stream. World can be set earlier
  in the command stream and then reused across mesh draws.

Active ASI change:

- Cache `D3DTS_WORLD` in `Hook_SetTransform`, alongside the existing
  view/projection cache.
- Resend world/view/projection before fixed-function indexed world draws via
  `ResendTransformsForDraw`.
- Existing guards remain: skip pretransformed UI, skip shader draws, and skip
  while current view is identity.

Reason:

- The user-visible artifact is global object/instance displacement while moving,
  not a tree texture issue.
- The previous ASI only resent view/projection. If Remix misses or coalesces the
  world state transition that TOD relies on, it can reconstruct unstable
  instances/transforms. Resending world with the camera state is a generic D3D9
  state fix, not a content/hash workaround.
...
- If moving displacement improves: keep world-transform resend and inspect logs
  for `worldSets` / `resends`.
- If unchanged: instrument per-draw transform fingerprints around the zipline
  scene instead of adding texture hashes.
- If it regresses UI/sky: check whether the pretransformed/identity-view guards
  missed a special non-RHW pass.

### 2026-06-05 17:48 - World resend rejected; Remix instance-stability diagnostic active

User result:

- World/view/projection resend did not fix the moving displacement/glitch.

Reverted:

- Removed the `D3DTS_WORLD` cache/resend change.
- Rebuilt ASI back to the previous camera-only resend source.

Important log clarification:

- `TOD_ENABLE_RUNTIME_LOG` is `0`, so the ASI runtime `Log(...)` calls after
  the logger definition compile out. A stale `tod-camera-resend.log` does not by
  itself prove the ASI is unloaded.

Remix source evidence:

- `rtx.enableAlwaysCalculateAABB` option description:
  calculates AABB for every draw call and may improve instance tracking across
  frames for skinned and vertex-shaded calls.
- `rtx.useBuffersDirectly` option description:
  when enabled, Remix uses incoming vertex buffers directly where possible.
  For a D3D9 game with mutable/shared dynamic buffers, disabling this is a clean
  way to test whether stale buffer reads / buffer aliasing are causing unstable
  instances.
...
- The symptom is still global moving geometry/instance displacement, not a
  texture-list problem.
- Since camera resend, world resend, temporal options, and RT-routing tests did
  not solve it, the next clean layer is Remix's instance tracking and geometry
  buffer ingestion.
...
- If this improves the moving glitch: split the two options next to identify
  whether AABB tracking or direct buffer use is responsible.
- If unchanged: inspect `d3d9.allowDiscard = False` and/or per-draw geometry
  hash/input stability. Do not add texture hashes.
- If performance drops but glitch improves: keep as evidence and look for a
  narrower buffer-copy rule.

### 2026-06-05 18:06 - AABB/direct-buffer diagnostic rejected; D3D9 discard-lock diagnostic active

User result:

- The moving/displaced geometry glitch is still present with:
...
- Removed both options from `rtx.conf` and `scripts\rtx.conf`.

Ghidra evidence refreshed:

- `FUN_004342c0` is still the render-command interpreter.
- `FUN_004540e0` is the central fixed-function indexed mesh draw:
  it binds stream source, FVF, indices, flushes texture state, then calls
  `DrawIndexedPrimitive`.
- `FUN_0044e220` is the central render-target/depth/viewport switcher.

Reason for next diagnostic:

- Remix-side AABB recomputation and geometry-copy mode did not fix the issue.
- The artifact is still global moving geometry/instance displacement, not a
  content texture problem.
- `d3d9.allowDiscard = False` changes DXVK's D3D9 lock behavior itself by
  stripping `D3DLOCK_DISCARD`, so it tests whether TOD's mutable dynamic buffers
  are being observed by Remix through unstable discard slices.
...
tools\tod_camera_resend_asi\TODCameraResend.cpp SHA256 60B7B9D98D144C6DCB77598BB3CA0BB6330432F0212928EBD09795C5963B6013
rtx.conf SHA256 8223AECE0E8E986E09A29B4CD4F1FFD056F1A34414FA8687ECDA6A899E8FF987
scripts\rtx.conf SHA256 A505525E28FB47A0B2737CF0F53E084C671F957AA633EEC339D31B3FCD01F4FF
dxvk.conf SHA256 4D764B01E44814687C14AFAA977E1B1A48A2CE02856ECB4CE45D5300E0A23533
scripts\dxvk.conf SHA256 4D764B01E44814687C14AFAA977E1B1A48A2CE02856ECB4CE45D5300E0A23533
```

Next result interpretation:

- If the moving glitch improves: keep `allowDiscard = False` as evidence and
  inspect TOD's dynamic vertex/index-buffer update functions for a narrower ASI
  or config-side fix.
- If unchanged: revert `allowDiscard = False` and test
  `rtx.enableInstanceDebuggingTools = True`, because Remix source says it
  disables temporal instance correlation.
- If performance drops but the artifact improves: the root is likely dynamic
  buffer aliasing/discard reuse, not textures, sky, or DLFG.

### 2026-06-05 18:10 - D3D9 discard-lock diagnostic rejected; instance correlation diagnostic active
...
- Removed `d3d9.allowDiscard = False` from `dxvk.conf` and `scripts\dxvk.conf`.
- Kept `d3d9.evictManagedOnUnlock = True`, because it is still part of the
  known working RTX/HUD baseline.

Active diagnostic config:

```ini
rtx.enableInstanceDebuggingTools = True
```

Reason:

- The failed `allowDiscard` result makes dynamic-buffer discard slices less
  likely as the only cause.
- Remix source indicates `enableInstanceDebuggingTools` disables temporal
  correlation for instances. The user-visible issue still looks like instance
  matching or previous-frame reconstruction picking the wrong transform/geometry
  while the camera moves.
- This is still a config-only diagnostic; no ASI behavior or texture lists were
  changed.
...
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
tools\tod_camera_resend_asi\TODCameraResend.cpp SHA256 60B7B9D98D144C6DCB77598BB3CA0BB6330432F0212928EBD09795C5963B6013
rtx.conf SHA256 ECAB6A3C1FD4E58537206E3F364CEDC792D679D260D39953198BEA275C8B8912
scripts\rtx.conf SHA256 0DDAC44C248BEFA2CA3CF38EBA1CD5F012B1750D9AF03F26D447EFBBD838C22A
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
```

Next result interpretation:

- If the moving glitch improves or changes shape: focus on Remix instance
  temporal correlation / geometry identity rules.
- If unchanged: revert `enableInstanceDebuggingTools` and inspect TOD's
  draw-command mesh identity inputs more directly. Likely next clean tests are
  geometry hash rule changes or a small ASI diagnostic that fingerprints
  stream/index/texture/transform identity without dumping textures.

### 2026-06-05 18:44 - Instance-debug flag fixed displacement but caused blur; unique-object radius test active

User result:

- With `rtx.enableInstanceDebuggingTools = True`, the texture/geometry
  displacement appears to be gone.
- New artifact: player/car get a blurry/soft temporal effect while moving.
- User videos:
  - `C:\Users\utkar\Videos\NVIDIA\Total Overdose\Total Overdose 2026.06.05 - 18.37.20.09.mp4`
  - `C:\Users\utkar\Videos\NVIDIA\Total Overdose\Total Overdose 2026.06.05 - 18.33.57.06.mp4`

Source evidence:

- `rtx.enableInstanceDebuggingTools` is documented in
  `third_party\dxvk-remix\src\dxvk\rtx_render\rtx_options.h` as disabling
  temporal correlation for instances, so it is too broad for a final fix.
- `DrawCallTracker::findOrCreateReplacementInstance` first exact-matches by full
  identity, then falls back to topology/material plus nearest world-space
  centroid within `rtx.uniqueObjectDistance`.
- Default `rtx.uniqueObjectDistance` is `300.f`; the option description says too
  high can make repeated objects flicker.

Interpretation:

- The debug flag result is a strong confirmation that the long-running moving
  glitch is wrong previous-frame instance matching, not texture hashing, sky,
  fog, DLFG, or ASI draw routing.
- The blur likely comes from throwing away too much useful temporal instance
  history globally.
...
Active narrower diagnostic/fix candidate:

```ini
rtx.uniqueObjectDistance = 75.0
```

Reason:

- Keeps temporal history enabled, but tightens the nearest-neighbor spatial
  matching radius from 300 units to 75 units.
- TOD has repeated modular world pieces, CPU-updated billboards, fast vehicles,
  and camera-relative motion; a 300-unit radius is likely too permissive.
- This is a native Remix instance-matching option, not a texture hash or ASI
  content hack.
...
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
tools\tod_camera_resend_asi\TODCameraResend.cpp SHA256 60B7B9D98D144C6DCB77598BB3CA0BB6330432F0212928EBD09795C5963B6013
rtx.conf SHA256 D897B4C41367825AD1F270FBE563E09E346E13A3C2A2A285B55D4BC99ECB6AA7
scripts\rtx.conf SHA256 80693D07F1CF21DB71AE1A8D1B973AAC2B6EC4FB03E1B17D415022980B9C4E4A
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
```
...
- If displacement stays fixed and blur improves: tune the radius (`50`, `100`,
  etc.) to find the largest stable value.
- If displacement returns: the radius is still too large or the exact-match path
  is the real issue; next test is a lower radius (`25`/`50`) or an ASI diagnostic
  that fingerprints draw identity.
- If fast vehicles/player start flickering: radius is too low; raise toward
  `100` or `150`.

### 2026-06-05 18:50 - `uniqueObjectDistance = 75` rejected; lowered to 25

User result:

- Moving displacement/glitch appears to be back with:
...
- `75` is still too permissive for TOD's repeated nearby geometry and
  camera-relative CPU-updated meshes.
- The previous `enableInstanceDebuggingTools=True` result still indicates the
  root layer is previous-frame instance matching, but the final fix should avoid
  disabling temporal correlation globally.

Active diagnostic/fix candidate:
...
```text
rtx.conf SHA256 372C5863FF0E72DA6C921360FC299A9829518174CC179FB87C8E7FFA6223FCFE
scripts\rtx.conf SHA256 449470DE08B605F222E0E1A23AF9261CE2C7F14120CDCCCA958B8C9D9DB552E9
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
```
...
- If fixed: tune upward/downward around `25` for best compromise.
- If still displaced: lower to `5.0` or near-zero, then decide whether the
  exact-match path or topology/material spatial matching needs a code-level
  workaround.
- If cars/player become too blurry/flickery: `25` is too low for fast movers;
  test `40` or `50`.

### 2026-06-05 18:54 - `uniqueObjectDistance = 25` good; testing 15

User result:

- `rtx.uniqueObjectDistance = 25.0` is good.
- User requested a tighter test at `15.0`.

Active config:

```ini
rtx.uniqueObjectDistance = 15.0
```

Reason:

- We are tuning the smallest stable spatial matching radius that preserves useful
  temporal history while preventing wrong previous-frame instance reuse.
- `75` was too high; `25` is currently the first good value.

Active state:

```text
rtx.conf SHA256 6CA30BF64B28E048E4CC1E30FD324CA3E217626401CCB42E250D3DFEE59ED9DF
scripts\rtx.conf SHA256 7DBDA853A75FC2EF200884E8F4931FD7E6B476B6A4038F2B710DE1D3FBBDF943
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
```
...
- If `15` is also good and not blurrier: keep it or test `10`.
- If `15` causes player/car blur/flicker: restore `25`.
- If no visible difference from `25`: prefer `25` as the less aggressive value.

### 2026-06-05 18:58 - `uniqueObjectDistance` compromise test: 20

User result:

- `25.0`: good overall, but minimal flickering remains.
- `15.0`: reduces the wrong-instance room further, but noticeable player/car
  blur appears, especially in vehicles.

Active config:

```ini
rtx.uniqueObjectDistance = 20.0
```

Reason:

- `20` is the midpoint between the two observed tradeoffs:
  less permissive than `25` for wrong previous-frame instance reuse, but less
  aggressive than `15` so fast movers should retain more temporal history.

Active state:

```text
rtx.conf SHA256 D17BBCD40FDFDE0FD215C1D6F16DFA8966FDA0A76BF6C1159AD5B36483B0090E
scripts\rtx.conf SHA256 35154567072560ED7D64DEA8753EC2233A313B61BFAEA416684B57E1E6BF6CA4
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
```
...
- If `20` keeps blur close to `25` while reducing flicker: keep `20`.
- If blur still looks like `15`: restore `25`.
- If flicker still looks like `25`: either keep `25` as the smoother value or
  test `18`.

### 2026-06-05 19:05 - Saved-value decision: restore `uniqueObjectDistance = 25`

User result:

- `25.0`: best confirmed value so far. Minor flicker remains, but acceptable.
- `15.0`: noticeable player/car blur, especially while driving.
- `20.0`: tested as compromise, but user asked to restore `25.0` for the saved
  state.
...
- The moving texture/geometry displacement is a Remix previous-frame instance
  matching issue.
- Broadly disabling instance temporal correlation with
  `rtx.enableInstanceDebuggingTools = True` removed displacement but introduced
  broad temporal blur, so it is not the final approach.
- `rtx.uniqueObjectDistance = 25.0` is the best native Remix compromise found so
  far: much tighter than the default `300.0`, while still giving vehicles/player
  enough temporal history to avoid the blur seen at `15.0`.

Active state for `state7`:

```text
rtx.conf SHA256 372C5863FF0E72DA6C921360FC299A9829518174CC179FB87C8E7FFA6223FCFE
scripts\rtx.conf SHA256 8927BCED897994EF0BEA262DA288AECF097540C24C18BCEDA352C7BC8B3910E0
dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
scripts\dxvk.conf SHA256 6544699D0C289971B8E9E76FF1ADF677EC19B6619689C4ED871D17026FE7857C
user.conf SHA256 7D19DB06905550895D368E41F429646E60CE22BB87A14A6CE59694FE2F8F0352
scripts\user.conf SHA256 24238055CF8821F0E58476CD8E8FD59F713E424CCB383B0DB77F9E48E91ADF51
scripts\TODCameraResend.asi SHA256 80EA0C88C16DABB1911C87034BC6D69DC37891F11626FE6C53B5077B5FD50AA3
tools\tod_camera_resend_asi\TODCameraResend.cpp SHA256 60B7B9D98D144C6DCB77598BB3CA0BB6330432F0212928EBD09795C5963B6013
```
...
```text
C:\GOG Games\TotalOverDoseRTXRemix\state7
```

## 2026-06-07 — Sun shadow: auto-aim subsystem, then injected sun retired

User report: "I need the sun to create shadows properly, I am getting 2 shadows at
this point opposite directions."

### Diagnosis

Live INI diagnostic (`scripts\TODCameraResend.ini` hot-reloads every ~1s): swung the
injected light's direction to the opposite hemisphere and watched which shadow moved.
Screenshots (`screenshots/2suns`, `…/NVIDIA`) showed a **crisp** shadow that obeyed the
INI (= our injected directional light) and a **soft** shadow that did not (= the
sky/skybox via Remix's environment probe).

### Auto-aim subsystem (principled fix attempt)

Rather than hand-tune the vector, made the ASI read where TOD renders the sun and aim the
light there automatically. Ghidra confirmed TOD's sun is a `LensFlareManager` emitter
placed at a world position (props `EmitterSize`/`OcclusionFactor`/`LineCheckLength`),
rendered as a camera-facing billboard — tracing the per-level emitter through the entity
system is a deep dig, so the ASI captures the billboard instead.

`TODCameraResend.cpp` additions (gated by `kEnableSunAutoAim`):
- Identify the sun sprite by Remix texture hash from INI `[Sun] SpriteHashHex`
  (`ComputeRtxTextureHash`, per-frame hash budget `kSunHashAttemptsPerFrame`, pointer
  cache so it is a pointer compare once resolved). Hash stays in config, never the binary.
- Read the billboard's object-space centroid from the bound VB (`ReadVbSunCenter`) or UP
  pointer (`ReadMemSunCenter`); cache `D3DTS_WORLD` (`g_lastWorld`) and apply it.
- Camera world pos from `g_lastView` via the rigid-inverse `eye = -t·Rᵀ`.
- Light dir = `-normalize(center - eye)`, latched (`g_sunAutoDir*`), used by
  `InjectSunLight` when valid; falls back to INI `Direction*` until the sun is first seen.
- `Log()` is compiled out in shipping builds, so added `WriteSunAutoAimRow` →
  `rtx-remix\logs\tod-sun-autoaim.log` for always-on verification.

Result: `RESOLVED sun sprite tex=… hash=0x5EF9EBC260F4B6BC size=64x64`, then a rock-stable
`lightDir=(-0.942,-0.216,0.259)` every frame. The capture/math work correctly.

### Key finding — the injected sun was redundant

Even perfectly auto-aimed at the lens-flare sun, **two shadows remained**. Zeroing the
injected light's diffuse (INI hot-reload) left a single, correct shadow: **TOD's sky/
skybox already casts the sun shadow via Remix's environment probe.** The injected light
was always a *second* light, and the lens-flare sun direction ≠ the skybox sun direction,
so it could never merge. The whole multi-session injected-sun effort was therefore
redundant for shadows.

### Resolution (user chose "keep skybox shadow")

- `kInjectSunLight = false` and `kEnableSunAutoAim = false` (constexpr → dead-code
  eliminated; ASI back to **140288 bytes**, `/W4` clean).
- INI `[Sun]` section marked INACTIVE; values kept for reference. Auto-aim machinery
  retained gated-off for a possible future crisp-directional-sun (which would also need
  `rtx.skyBrightness` lowered so the skybox shadow does not double up).

### Darker shadows (follow-up request)

`rtx.tonemappingMode` defaults to **Local**, so the global tonemapper's `dynamicRange` is
inert. The live lever is `rtx.localtonemap.shadows` (default `2.0`, *lower = darker*; a
local operator, so it deepens shaded areas without dimming the bright sky/road).

```ini
rtx.localtonemap.shadows = 1.4
```

Tunable live in the Alt+X menu (Local Tonemapping → Shadows). Note: only `scripts\rtx.conf`
exists now — no root `rtx.conf` mirror to maintain.
