...
constexpr bool kMarkTodSkyDrawsWithViewportMinZ = false;
...
constexpr float kTodCameraFarClipOverride = 3000.0f;
...
bool TodSkyTextureRecordExists(
  ...
  for (LONG i = 0; i < count; ++i) {
    const TodSkyTextureRecord& record = g_todSkyTextureRecords[i];
  ...
  for (LONG i = 0; i < pendingCount; ++i) {
    ...
    if (hashTexture == nullptr || info == nullptr || hashTexture->GetType() != D3DRTYPE_TEXTURE) {
      if (!TodSkyTextureRecordExists(hashTexture, 0, false)) {
        AddTodSkyTextureRecord(hashTexture, 0, false);
    ...
    if (!TodSkyTextureRecordExists(hashTexture, rtxHash, hashValid)) {
      AddTodSkyTextureRecord(hashTexture, rtxHash, hashValid);
      if (hashValid) {
...
HRESULT APIENTRY Hook_DrawIndexedPrimitive(
  ...
  if (todSkyDraw) {
    ...
    if (skyDraws <= 10 || (skyDraws % 300) == 0) {
      Log(
          "TOD SkyBox indexed draw #%ld: primCount=%u numVertices=%u viewportMarked=%d",
          skyDraws,
          primitiveCount,
          numVertices,
