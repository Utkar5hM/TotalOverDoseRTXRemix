...
constexpr DWORD kDeviceSetSamplerStateIndex = 69;
...
// Direction/color are runtime-tunable from scripts/TODRemixShim.ini while we
...
// sprite is identified by its Remix texture hash from scripts/TODRemixShim.ini
...
constexpr float kDefaultTodCameraFarClipOverride = 8000.0f;
constexpr float kDefaultTodLodFactorOverride = 4.0f;
constexpr float kDefaultTodMinimumTraverseDistance = 200.0f;
constexpr float kDefaultTodTextureMipBias = -2.0f;
constexpr DWORD kDefaultTodTextureMaxAnisotropy = 16;
constexpr DWORD kDefaultTodTextureSamplerCount = 4;
...
constexpr uintptr_t kTodLodFactorGlobalVa = 0x00A12090u;
constexpr uintptr_t kTodLodFactorGlobalRva = kTodLodFactorGlobalVa - kTodImageBaseVa;
constexpr uintptr_t kTodAssetManagerGlobalVa = 0x00A3D7C4u;
constexpr uintptr_t kTodAssetManagerGlobalRva =
    kTodAssetManagerGlobalVa - kTodImageBaseVa;
constexpr uintptr_t kTodFolderTypeGlobalVa = 0x00A3D810u;
constexpr uintptr_t kTodFolderTypeGlobalRva =
    kTodFolderTypeGlobalVa - kTodImageBaseVa;
constexpr uintptr_t kTodAssetManagerFindFirstEntityVa = 0x008755E0u;
constexpr uintptr_t kTodAssetManagerFindFirstEntityRva =
    kTodAssetManagerFindFirstEntityVa - kTodImageBaseVa;
constexpr uintptr_t kTodAssetManagerFindNextEntityVa = 0x00875610u;
constexpr uintptr_t kTodAssetManagerFindNextEntityRva =
    kTodAssetManagerFindNextEntityVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneLoadMapVa = 0x008932A0u;
constexpr uintptr_t kTodSceneLoadMapRva = kTodSceneLoadMapVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneUpdateLoadedBlocksVa = 0x008986E0u;
constexpr uintptr_t kTodSceneUpdateLoadedBlocksRva =
    kTodSceneUpdateLoadedBlocksVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneUpdateVa = 0x00897450u;
constexpr uintptr_t kTodSceneUpdateRva = kTodSceneUpdateVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneDestroyVa = 0x00895E40u;
constexpr uintptr_t kTodSceneDestroyRva = kTodSceneDestroyVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneStartVa = 0x0089A100u;
constexpr uintptr_t kTodSceneStartRva = kTodSceneStartVa - kTodImageBaseVa;
constexpr uintptr_t kTodSceneResetVa = 0x0089A1A0u;
constexpr uintptr_t kTodSceneResetRva = kTodSceneResetVa - kTodImageBaseVa;
constexpr uintptr_t kTodKapowEngineOpenSceneVa = 0x0093CE00u;
constexpr uintptr_t kTodKapowEngineOpenSceneRva =
    kTodKapowEngineOpenSceneVa - kTodImageBaseVa;
...
constexpr uintptr_t kTodSetTraverseDistanceVa = 0x00500BF0u;
constexpr uintptr_t kTodSetTraverseDistanceRva =
    kTodSetTraverseDistanceVa - kTodImageBaseVa;
...
constexpr LONG kMaxTodAssetCaptureLodFactors = 8;
constexpr LONG kMaxTodAssetCrawlerEntries = 1024;
...
constexpr SIZE_T kTodSetTraverseDistancePatchBytes = 5;
...
using CreateTextureFn = HRESULT(APIENTRY*)(
    ...
    HANDLE* sharedHandle);
using TextureReleaseFn = ULONG(APIENTRY*)(IDirect3DBaseTexture9* self);
...
using SetSamplerStateFn = HRESULT(APIENTRY*)(
    IDirect3DDevice9* self,
    DWORD sampler,
...
enum TodAssetCaptureState {
  kTodAssetCaptureIdle = 0,
  kTodAssetCaptureStartDelay = 1,
  kTodAssetCaptureSettle = 2,
  kTodAssetCaptureProbe = 3,
};

enum TodAssetCrawlerState {
  kTodAssetCrawlerIdle = 0,
  kTodAssetCrawlerLoadEntry = 1,
  kTodAssetCrawlerSettle = 2,
  kTodAssetCrawlerWaitForProbe = 3,
};

struct TodAssetCaptureConfig {
  bool enabled;
  LONG startDelayFrames;
  LONG settleFrames;
  LONG probeFrames;
  LONG lodFactorCount;
  float lodFactors[kMaxTodAssetCaptureLodFactors];
};

struct TodAssetCrawlerConfig {
  bool enabled;
  bool includeMaps;
  bool includeMissions;
  bool includeCutscenes;
  bool includePlayerdata;
  LONG loadSettleFrames;
};

struct TodAssetCrawlerEntry {
  void* folder;
  LONG blockId;
  char name[128];
};
...
struct TodRenderDistanceConfig {
  bool enabled;
  float lodFactor;
  float farClip;
  float minimumTraverseDistance;
};

struct TodTextureQualityConfig {
  bool enabled;
  float mipBias;
  DWORD maxAnisotropy;
  DWORD samplerCount;
...
char g_todAssetCaptureLogPath[MAX_PATH * 2] = {};
char g_todAssetCrawlerLogPath[MAX_PATH * 2] = {};
...
SetSamplerStateFn g_origSetSamplerState = nullptr;
...
TodRenderDistanceConfig g_todRenderDistanceConfig = {
    true,
    kDefaultTodLodFactorOverride,
    kDefaultTodCameraFarClipOverride,
    kDefaultTodMinimumTraverseDistance,
};
DWORD g_todRenderDistanceConfigLastReadTick = 0;
TodTextureQualityConfig g_todTextureQualityConfig = {
    true,
    kDefaultTodTextureMipBias,
    kDefaultTodTextureMaxAnisotropy,
    kDefaultTodTextureSamplerCount,
    false,
};
DWORD g_todTextureQualityConfigLastReadTick = 0;
DWORD g_deviceMaxAnisotropy = 1;
float g_todRenderDistanceOriginalLodFactor = 1.0f;
bool g_todRenderDistanceOriginalLodFactorValid = false;
TodAssetCaptureConfig g_todAssetCaptureConfig = {
    false,
    180,
    120,
    3,
...
TodAssetCrawlerConfig g_todAssetCrawlerConfig = {
    false,
    true,
    true,
    false,
...
TodAssetCrawlerEntry g_todAssetCrawlerEntries[kMaxTodAssetCrawlerEntries] = {};
DWORD g_todAssetCaptureConfigLastReadTick = 0;
DWORD g_todAssetCrawlerConfigLastReadTick = 0;
volatile LONG g_todAssetCaptureState = 0;
volatile LONG g_todAssetCaptureArmed = 0;
volatile LONG g_todAssetCaptureFramesRemaining = 0;
volatile LONG g_todAssetCaptureLodIndex = 0;
volatile LONG g_todAssetCaptureProbeFrame = 0;
float g_todAssetCaptureSavedLodFactor = 1.0f;
bool g_todAssetCaptureSavedLodFactorValid = false;
volatile LONG g_todAssetCrawlerState = 0;
volatile LONG g_todAssetCrawlerArmed = 0;
volatile LONG g_todAssetCrawlerEntryCount = 0;
volatile LONG g_todAssetCrawlerEntryIndex = 0;
volatile LONG g_todAssetCrawlerFramesRemaining = 0;
volatile LONG g_todAssetCrawlerProbeRequested = 0;
volatile LONG g_todAssetCrawlerBootstrapped = 0;
...
TodSceneUpdateFn g_origTodSceneUpdate = nullptr;
TodSetTraverseDistanceFn g_origTodSetTraverseDistance = nullptr;
...
volatile LONG g_todSceneUpdateHooked = 0;
volatile LONG g_todSetTraverseDistanceHooked = 0;
...
void InitializePaths(HMODULE module) {
  ...
  snprintf(
      g_logPath,
      sizeof(g_logPath),
      "%s\\rtx-remix\\logs\\tod-remix-shim.log",
      g_gameRoot);
  snprintf(
      g_sunConfigPath,
      sizeof(g_sunConfigPath),
      "%s\\scripts\\TODRemixShim.ini",
      g_gameRoot);
  snprintf(
  ...
  snprintf(
      ...
      g_gameRoot);
  snprintf(
      g_todAssetCaptureLogPath,
      sizeof(g_todAssetCaptureLogPath),
      "%s\\rtx-remix\\logs\\tod-asset-capture.log",
      g_gameRoot);
  snprintf(
      g_todAssetCrawlerLogPath,
      sizeof(g_todAssetCrawlerLogPath),
      "%s\\rtx-remix\\logs\\tod-asset-crawler.log",
      g_gameRoot);
}
...
void Log(const char* fmt, ...) {
  ...
  HANDLE file = CreateFileA(
      g_logPath[0] != '\0' ? g_logPath : "rtx-remix\\logs\\tod-remix-shim.log",
      FILE_APPEND_DATA,
...
void WriteTodAssetCaptureStatus(const char* fmt, ...) {
  char message[512] = {};
  va_list args;
  va_start(args, fmt);
...
void WriteTodAssetCrawlerStatus(const char* fmt, ...) {
  char message[512] = {};
  va_list args;
  va_start(args, fmt);
  vsnprintf_s(message, sizeof(message), _TRUNCATE, fmt, args);
...
void RefreshTodRenderDistanceConfig() {
  const DWORD now = GetTickCount();
  if (g_todRenderDistanceConfigLastReadTick != 0 &&
      now - g_todRenderDistanceConfigLastReadTick < 1000) {
    return;
  }
  g_todRenderDistanceConfigLastReadTick = now;

  TodRenderDistanceConfig next = g_todRenderDistanceConfig;
  next.enabled =
      GetPrivateProfileIntA("RenderDistance", "Enabled", 1, g_sunConfigPath) != 0;
  next.lodFactor = ClampFloat(
      ReadIniFloat(
          "RenderDistance",
          "LodFactor",
          kDefaultTodLodFactorOverride),
  ...
  next.farClip = ClampFloat(
      ...
      100.0f,
      10000.0f);
  next.minimumTraverseDistance = ClampFloat(
      ReadIniFloat(
          "RenderDistance",
          "MinimumTraverseDistance",
          kDefaultTodMinimumTraverseDistance),
      0.0f,
      2000.0f);
  g_todRenderDistanceConfig = next;
}

void RefreshTodTextureQualityConfig() {
  const DWORD now = GetTickCount();
  if (g_todTextureQualityConfigLastReadTick != 0 &&
      now - g_todTextureQualityConfigLastReadTick < 1000) {
    return;
  }
  g_todTextureQualityConfigLastReadTick = now;

  TodTextureQualityConfig next = g_todTextureQualityConfig;
  next.enabled =
      GetPrivateProfileIntA("TextureQuality", "Enabled", 1, g_sunConfigPath) != 0;
  next.mipBias = ClampFloat(
      ReadIniFloat(
          "TextureQuality",
          "MipBias",
          kDefaultTodTextureMipBias),
      -4.0f,
      0.0f);
  next.maxAnisotropy = static_cast<DWORD>(
  ...
  if (next.maxAnisotropy < 1) {
    next.maxAnisotropy = 1;
  } else if (next.maxAnisotropy > 16) {
    next.maxAnisotropy = 16;
  }
  next.samplerCount = static_cast<DWORD>(
      GetPrivateProfileIntA(
          "TextureQuality",
          "SamplerCount",
          kDefaultTodTextureSamplerCount,
          g_sunConfigPath));
  if (next.samplerCount < 1) {
    next.samplerCount = 1;
  } else if (next.samplerCount > 8) {
    next.samplerCount = 8;
  }
  next.forceTopMip =
      GetPrivateProfileIntA("TextureQuality", "ForceTopMip", 0, g_sunConfigPath) != 0;
  g_todTextureQualityConfig = next;
}

LONG ReadClampedIniInt(
    const char* section,
    const char* key,
    LONG defaultValue,
    LONG minimum,
    LONG maximum) {
  const int value = GetPrivateProfileIntA(section, key, defaultValue, g_sunConfigPath);
  if (value < minimum) {
    return minimum;
  }
  if (value > maximum) {
...
void RefreshTodAssetCaptureConfig() {
  const DWORD now = GetTickCount();
  if (g_todAssetCaptureConfigLastReadTick != 0 &&
      now - g_todAssetCaptureConfigLastReadTick < 1000) {
    return;
  }
  g_todAssetCaptureConfigLastReadTick = now;

  TodAssetCaptureConfig next = g_todAssetCaptureConfig;
  next.enabled =
      GetPrivateProfileIntA("AssetCapture", "Enabled", 0, g_sunConfigPath) != 0;
  next.startDelayFrames =
      ReadClampedIniInt("AssetCapture", "StartDelayFrames", 180, 0, 36000);
  next.settleFrames =
      ReadClampedIniInt("AssetCapture", "SettleFrames", 120, 1, 36000);
  next.probeFrames =
      ReadClampedIniInt("AssetCapture", "ProbeFrames", 3, 1, 60);
  ...
  float parsedFactors[kMaxTodAssetCaptureLodFactors] = {};
  LONG parsedCount = 0;
  char* context = nullptr;
  for (char* token = strtok_s(factorText, ",; \t", &context);
       token != nullptr && parsedCount < kMaxTodAssetCaptureLodFactors;
       token = strtok_s(nullptr, ",; \t", &context)) {
    char* end = nullptr;
    const float factor = static_cast<float>(strtod(token, &end));
    if (end != token && IsFiniteFloat(factor) && factor >= 0.001f && factor <= 100.0f) {
      parsedFactors[parsedCount++] = factor;
    }
  }
  if (parsedCount > 0) {
    next.lodFactorCount = parsedCount;
    for (LONG i = 0; i < parsedCount; ++i) {
      next.lodFactors[i] = parsedFactors[i];
    }
  }

  g_todAssetCaptureConfig = next;
}

void RefreshTodAssetCrawlerConfig() {
  const DWORD now = GetTickCount();
  if (g_todAssetCrawlerConfigLastReadTick != 0 &&
      now - g_todAssetCrawlerConfigLastReadTick < 1000) {
    return;
  }
  g_todAssetCrawlerConfigLastReadTick = now;

  TodAssetCrawlerConfig next = g_todAssetCrawlerConfig;
  next.enabled =
      GetPrivateProfileIntA("AssetCrawler", "Enabled", 0, g_sunConfigPath) != 0;
  next.includeMaps =
      GetPrivateProfileIntA("AssetCrawler", "IncludeMaps", 1, g_sunConfigPath) != 0;
  next.includeMissions =
      GetPrivateProfileIntA("AssetCrawler", "IncludeMissions", 1, g_sunConfigPath) != 0;
  next.includeCutscenes =
      GetPrivateProfileIntA("AssetCrawler", "IncludeCutscenes", 0, g_sunConfigPath) != 0;
  next.includePlayerdata =
      GetPrivateProfileIntA("AssetCrawler", "IncludePlayerdata", 1, g_sunConfigPath) != 0;
  next.loadSettleFrames =
      ReadClampedIniInt("AssetCrawler", "LoadSettleFrames", 300, 30, 36000);
  g_todAssetCrawlerConfig = next;
}
...
bool PatchTodCameraFarClip(uintptr_t camera) {
  if (!kPatchTodCameraFarClip || !g_todRenderDistanceConfig.enabled || camera == 0) {
    return false;
  }
  ...
  const float farClipOverride = g_todRenderDistanceConfig.farClip;
  if (farClip >= farClipOverride - 1.0f) {
    return false;
  }

  *reinterpret_cast<float*>(farClipAddress) = farClipOverride;
...
void ApplyTodCameraFarClipOverride() {
  RefreshTodRenderDistanceConfig();
...
SurfaceInfo* RegisterSurface(
  ...
  for (LONG i = 0; i < count; ++i) {
    ...
    if (emptyIndex < 0 && g_surfaceInfo[i].surface == nullptr) {
      emptyIndex = i;
    }
  }

  LONG index = emptyIndex;
  if (index < 0 && count >= maxCount) {
    return nullptr;
  }
  if (index < 0) {
    index = InterlockedIncrement(const_cast<LONG*>(&g_surfaceInfoCount)) - 1;
  }
...
float* TodLodFactorAddress() {
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t address = moduleBase + kTodLodFactorGlobalRva;
  if (moduleBase == 0 || !IsCommittedMemoryRange(address, sizeof(float), true)) {
    return nullptr;
  }
  return reinterpret_cast<float*>(address);
}

void ApplyTodLodFactorOverride() {
  RefreshTodRenderDistanceConfig();

  float* lodFactor = TodLodFactorAddress();
  if (lodFactor == nullptr ||
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCaptureState), 0, 0) !=
          kTodAssetCaptureIdle) {
    return;
  }

  if (!g_todRenderDistanceConfig.enabled) {
    if (g_todRenderDistanceOriginalLodFactorValid) {
      *lodFactor = g_todRenderDistanceOriginalLodFactor;
      g_todRenderDistanceOriginalLodFactorValid = false;
    }
    return;
  }

  if (!g_todRenderDistanceOriginalLodFactorValid) {
    g_todRenderDistanceOriginalLodFactor = *lodFactor;
    g_todRenderDistanceOriginalLodFactorValid = true;
  }

  if (fabsf(*lodFactor - g_todRenderDistanceConfig.lodFactor) > 0.0001f) {
    *lodFactor = g_todRenderDistanceConfig.lodFactor;
  }
}

void RestoreTodAssetCaptureLodFactor() {
  float* lodFactor = TodLodFactorAddress();
  if (g_todAssetCaptureSavedLodFactorValid && lodFactor != nullptr) {
    *lodFactor = g_todAssetCaptureSavedLodFactor;
  }
  g_todAssetCaptureSavedLodFactorValid = false;
}

void ResetTodAssetCaptureRecords() {
  LONG pendingCount =
      InterlockedExchange(const_cast<LONG*>(&g_todForcedTexturePendingCount), 0);
  if (pendingCount > kMaxTodForcedTexturePending) {
    pendingCount = kMaxTodForcedTexturePending;
  }
  for (LONG i = 0; i < pendingCount; ++i) {
    if (g_todForcedTexturePending[i].sourceTexture != nullptr) {
      g_todForcedTexturePending[i].sourceTexture->Release();
    }
  }

  ZeroMemory(g_todForcedTexturePending, sizeof(g_todForcedTexturePending));
  ZeroMemory(g_todForcedTextureRecords, sizeof(g_todForcedTextureRecords));
  InterlockedExchange(const_cast<LONG*>(&g_todForcedTextureRecordCount), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todForcedTextureRows), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todTextureMapProbeActive), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureProbeFrame), 0);
}

void StopTodAssetCapture(const char* status) {
  ProcessQueuedTodForcedTextureProbeTextures();
  RestoreTodAssetCaptureLodFactor();
  InterlockedExchange(const_cast<LONG*>(&g_todTextureMapProbeActive), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureState), kTodAssetCaptureIdle);
  WriteTodAssetCaptureStatus(
      "%s uniqueTextures=%ld rows=%ld",
      status,
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todForcedTextureRecordCount), 0, 0),
      InterlockedCompareExchange(const_cast<LONG*>(&g_todForcedTextureRows), 0, 0));
}

bool BeginTodAssetCapture() {
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t drawAllTextures = moduleBase + kTodTextureDrawAllTexturesRva;
  float* lodFactor = TodLodFactorAddress();
  if (moduleBase == 0 || lodFactor == nullptr ||
      !IsCommittedMemoryRange(drawAllTextures, 16, false)) {
    WriteTodAssetCaptureStatus("start_failed TOD addresses unavailable");
    return false;
  }

  ResetTodAssetCaptureRecords();
  g_todAssetCaptureSavedLodFactor = *lodFactor;
  g_todAssetCaptureSavedLodFactorValid = true;
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureLodIndex), 0);
  InterlockedExchange(
      const_cast<LONG*>(&g_todAssetCaptureFramesRemaining),
      g_todAssetCaptureConfig.startDelayFrames);
  InterlockedExchange(
      const_cast<LONG*>(&g_todAssetCaptureState),
      kTodAssetCaptureStartDelay);
  WriteTodAssetCaptureStatus(
      "started originalLod=%.6f factors=%ld delay=%ld settle=%ld probeFrames=%ld",
      g_todAssetCaptureSavedLodFactor,
      g_todAssetCaptureConfig.lodFactorCount,
      g_todAssetCaptureConfig.startDelayFrames,
      g_todAssetCaptureConfig.settleFrames,
      g_todAssetCaptureConfig.probeFrames);
  return true;
}

void ApplyCurrentTodAssetCaptureLodFactor() {
  const LONG index =
      InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCaptureLodIndex), 0, 0);
  float* lodFactor = TodLodFactorAddress();
  if (lodFactor == nullptr || index < 0 || index >= g_todAssetCaptureConfig.lodFactorCount) {
    StopTodAssetCapture("failed_invalid_lod_state");
    return;
  }

  *lodFactor = g_todAssetCaptureConfig.lodFactors[index];
  InterlockedExchange(
      const_cast<LONG*>(&g_todAssetCaptureFramesRemaining),
      g_todAssetCaptureConfig.settleFrames);
  InterlockedExchange(
      const_cast<LONG*>(&g_todAssetCaptureState),
      kTodAssetCaptureSettle);
  WriteTodAssetCaptureStatus(
      "lod_factor index=%ld value=%.6f",
      index,
      g_todAssetCaptureConfig.lodFactors[index]);
}

void RunTodTextureMapProbe() {
  RefreshTodAssetCaptureConfig();
  const bool crawlerRequested =
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerProbeRequested), 0, 0) != 0;
  const bool captureRequested = g_todAssetCaptureConfig.enabled || crawlerRequested;

  if (!captureRequested) {
    if (InterlockedCompareExchange(
            const_cast<LONG*>(&g_todAssetCaptureState), 0, 0) != kTodAssetCaptureIdle) {
      StopTodAssetCapture("disabled_restored");
    }
    InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureArmed), 0);
    return;
  }

  if (InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCaptureArmed), 1, 0) == 0) {
    if (!BeginTodAssetCapture()) {
      InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureArmed), 1);
      return;
    }
  }

  const LONG state =
      InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCaptureState), 0, 0);
  if (state == kTodAssetCaptureIdle) {
    return;
  }

  if (state == kTodAssetCaptureStartDelay || state == kTodAssetCaptureSettle) {
    const LONG remaining =
        InterlockedDecrement(const_cast<LONG*>(&g_todAssetCaptureFramesRemaining));
    if (remaining > 0) {
      return;
    }
    if (state == kTodAssetCaptureStartDelay) {
      ApplyCurrentTodAssetCaptureLodFactor();
    } else {
      InterlockedExchange(
          const_cast<LONG*>(&g_todAssetCaptureFramesRemaining),
          g_todAssetCaptureConfig.probeFrames);
      InterlockedExchange(
          const_cast<LONG*>(&g_todAssetCaptureState),
          kTodAssetCaptureProbe);
    }
    return;
  }

  if (state != kTodAssetCaptureProbe) {
    StopTodAssetCapture("failed_unknown_state");
    return;
  }
  ...
  if (moduleBase == 0 || !IsCommittedMemoryRange(functionAddress, 16, false)) {
    StopTodAssetCapture("failed_draw_all_unavailable");
    return;
  }

  const LONG probeFrame =
      InterlockedIncrement(const_cast<LONG*>(&g_todAssetCaptureProbeFrame));
  ...
  const LONG nextIndex =
      InterlockedIncrement(const_cast<LONG*>(&g_todAssetCaptureLodIndex));
  if (nextIndex >= g_todAssetCaptureConfig.lodFactorCount) {
    StopTodAssetCapture("completed_restored");
    return;
  }
  ApplyCurrentTodAssetCaptureLodFactor();
}

void* TodGlobalPointer(uintptr_t rva) {
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t address = moduleBase + rva;
  if (moduleBase == 0 || !IsCommittedMemoryRange(address, sizeof(void*), false)) {
    return nullptr;
  }
  return *reinterpret_cast<void**>(address);
}

LONG TodFolderBlockId(void* folder) {
  const uintptr_t address = reinterpret_cast<uintptr_t>(folder) + 0x50u;
  if (folder == nullptr || !IsCommittedMemoryRange(address, sizeof(LONG), false)) {
    return 0;
  }
  const LONG encoded = *reinterpret_cast<const LONG*>(address);
  return (encoded << 3) >> 3;
}

bool TodEntityIsFolder(void* entity, void* folderType) {
  if (entity == nullptr || folderType == nullptr) {
    return false;
  }

  const uintptr_t typeAddress = reinterpret_cast<uintptr_t>(entity) + 4u;
  if (!IsCommittedMemoryRange(typeAddress, sizeof(void*), false)) {
    return false;
  }

  void* type = *reinterpret_cast<void**>(typeAddress);
  for (LONG depth = 0; type != nullptr && depth < 64; ++depth) {
    if (type == folderType) {
      return true;
    }
    const uintptr_t parentAddress = reinterpret_cast<uintptr_t>(type) + 0x24u;
    if (!IsCommittedMemoryRange(parentAddress, sizeof(void*), false)) {
      return false;
    }
    type = *reinterpret_cast<void**>(parentAddress);
  }
  return false;
}

void CopyTodEntityName(void* entity, char* output, size_t outputSize) {
  if (output == nullptr || outputSize == 0) {
    return;
  }
  output[0] = '\0';
  const uintptr_t nameAddress = reinterpret_cast<uintptr_t>(entity) + 0x4Cu;
  if (entity == nullptr || !IsCommittedMemoryRange(nameAddress, sizeof(char*), false)) {
    return;
  }
  const char* name = *reinterpret_cast<const char* const*>(nameAddress);
  if (name == nullptr) {
    return;
  }
  size_t length = 0;
  while (length + 1 < outputSize &&
         IsCommittedMemoryRange(reinterpret_cast<uintptr_t>(name + length), 1, false) &&
         name[length] != '\0') {
...
bool TodAssetCrawlerIncludesBlockId(LONG blockId) {
  if (blockId == 1 || blockId == 2) {
    return g_todAssetCrawlerConfig.includeMaps;
  }
  if (blockId == 3) {
    return g_todAssetCrawlerConfig.includeMissions;
  }
  if (blockId == 4) {
    return g_todAssetCrawlerConfig.includeCutscenes;
  }
  if (blockId == 5) {
    return g_todAssetCrawlerConfig.includePlayerdata;
  }
  return false;
}

bool BuildTodAssetCrawlerCatalog() {
  const uintptr_t moduleBase = TodModuleBase();
  void* assetManager = TodGlobalPointer(kTodAssetManagerGlobalRva);
  void* folderType = TodGlobalPointer(kTodFolderTypeGlobalRva);
  const uintptr_t firstAddress = moduleBase + kTodAssetManagerFindFirstEntityRva;
  const uintptr_t nextAddress = moduleBase + kTodAssetManagerFindNextEntityRva;
  if (moduleBase == 0 || assetManager == nullptr || folderType == nullptr ||
      !IsCommittedMemoryRange(firstAddress, 16, false) ||
      !IsCommittedMemoryRange(nextAddress, 16, false)) {
    WriteTodAssetCrawlerStatus("catalog_failed engine registry unavailable");
    return false;
  }

  ZeroMemory(g_todAssetCrawlerEntries, sizeof(g_todAssetCrawlerEntries));
  LONG count = 0;
  TodFindFirstEntityFn findFirst =
      reinterpret_cast<TodFindFirstEntityFn>(firstAddress);
  TodFindNextEntityFn findNext =
      reinterpret_cast<TodFindNextEntityFn>(nextAddress);

  for (LONG wantedBlockId = 1; wantedBlockId <= 5; ++wantedBlockId) {
    if (!TodAssetCrawlerIncludesBlockId(wantedBlockId)) {
      continue;
    }
    void* entity = findFirst(assetManager);
    LONG guard = 0;
    while (entity != nullptr && guard++ < 65536 && count < kMaxTodAssetCrawlerEntries) {
      if (TodEntityIsFolder(entity, folderType) &&
          TodFolderBlockId(entity) == wantedBlockId) {
        TodAssetCrawlerEntry& entry = g_todAssetCrawlerEntries[count++];
        entry.folder = entity;
        entry.blockId = wantedBlockId;
        CopyTodEntityName(entity, entry.name, sizeof(entry.name));
        if (entry.name[0] == '\0') {
          snprintf(entry.name, sizeof(entry.name), "unnamed_block_%ld", count);
        }
      }
      entity = findNext(assetManager, entity);
    }
  }

  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerEntryCount), count);
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerEntryIndex), 0);
  WriteTodAssetCrawlerStatus(
      "catalog_ready entries=%ld maps=%d missions=%d cutscenes=%d playerdata=%d",
      count,
      g_todAssetCrawlerConfig.includeMaps ? 1 : 0,
      g_todAssetCrawlerConfig.includeMissions ? 1 : 0,
      g_todAssetCrawlerConfig.includeCutscenes ? 1 : 0,
      g_todAssetCrawlerConfig.includePlayerdata ? 1 : 0);
  return count > 0;
}

bool BootstrapTodAssetCrawlerScene(void* currentScene) {
  if (!g_todAssetCrawlerConfig.enabled ||
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerBootstrapped), 1, 0) != 0) {
    return false;
  }
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t destroyAddress = moduleBase + kTodSceneDestroyRva;
  const uintptr_t openSceneAddress = moduleBase + kTodKapowEngineOpenSceneRva;
  const uintptr_t startAddress = moduleBase + kTodSceneStartRva;
  const uintptr_t resetAddress = moduleBase + kTodSceneResetRva;
  if (currentScene == nullptr || moduleBase == 0 ||
      !IsCommittedMemoryRange(destroyAddress, 16, false) ||
      !IsCommittedMemoryRange(openSceneAddress, 16, false) ||
      !IsCommittedMemoryRange(startAddress, 16, false) ||
      !IsCommittedMemoryRange(resetAddress, 16, false)) {
    InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerBootstrapped), 0);
    WriteTodAssetCrawlerStatus("scene_bootstrap_unavailable");
    return false;
  }

  WriteTodAssetCrawlerStatus("scene_bootstrap_start path=/data/Overdose_THE_GAME/Overdose.scene");
  __try {
    WriteTodAssetCrawlerStatus("scene_bootstrap_reset");
    reinterpret_cast<TodSceneResetFn>(resetAddress)(currentScene);
    WriteTodAssetCrawlerStatus("scene_bootstrap_destroy");
    reinterpret_cast<TodSceneDestroyFn>(destroyAddress)(currentScene);
    WriteTodAssetCrawlerStatus("scene_bootstrap_open");
    const int opened = reinterpret_cast<TodKapowEngineOpenSceneFn>(openSceneAddress)(
        nullptr,
        "/data/Overdose_THE_GAME/Overdose.scene");
    void* scene = TodGlobalPointer(kTodCameraSystemGlobalRva);
    if (opened == 0 || scene == nullptr) {
      WriteTodAssetCrawlerStatus(
          "scene_bootstrap_failed opened=%d scene=%p",
          opened,
          scene);
      return true;
    }
    WriteTodAssetCrawlerStatus("scene_bootstrap_start_scene scene=%p", scene);
    reinterpret_cast<TodSceneStartFn>(startAddress)(scene);
  } __except(EXCEPTION_EXECUTE_HANDLER) {
    WriteTodAssetCrawlerStatus(
        "scene_bootstrap_exception code=0x%08lx",
        static_cast<unsigned long>(GetExceptionCode()));
    return true;
  }

  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerArmed), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerState), kTodAssetCrawlerIdle);
  WriteTodAssetCrawlerStatus("scene_bootstrap_complete");
  return true;
}

bool LoadTodAssetCrawlerEntry(void* scene, const TodAssetCrawlerEntry& entry) {
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t loadMapAddress = moduleBase + kTodSceneLoadMapRva;
  const uintptr_t updateBlocksAddress = moduleBase + kTodSceneUpdateLoadedBlocksRva;
  if (scene == nullptr || entry.folder == nullptr ||
      entry.blockId < 1 || entry.blockId > 5 ||
      moduleBase == 0 || !IsCommittedMemoryRange(loadMapAddress, 16, false) ||
      !IsCommittedMemoryRange(updateBlocksAddress, 16, false)) {
    return false;
  }

  __try {
    reinterpret_cast<TodSceneLoadMapFn>(loadMapAddress)(
        scene,
        static_cast<unsigned int>(entry.blockId - 1),
        entry.folder);
    reinterpret_cast<TodSceneUpdateLoadedBlocksFn>(updateBlocksAddress)(scene, 1, nullptr);
  } __except(EXCEPTION_EXECUTE_HANDLER) {
    WriteTodAssetCrawlerStatus(
        "load_exception index=%ld blockId=%ld code=0x%08lx name=%s",
        InterlockedCompareExchange(
            const_cast<LONG*>(&g_todAssetCrawlerEntryIndex), 0, 0),
        entry.blockId,
        static_cast<unsigned long>(GetExceptionCode()),
        entry.name);
    return false;
...
void StopTodAssetCrawler(const char* status) {
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerProbeRequested), 0);
  InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerState), kTodAssetCrawlerIdle);
  WriteTodAssetCrawlerStatus(
      "%s completed=%ld total=%ld",
      status,
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerEntryIndex), 0, 0),
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerEntryCount), 0, 0));
}

void RunTodAssetCrawlerAtSceneUpdate(void* scene) {
  RefreshTodAssetCrawlerConfig();
  if (!g_todAssetCrawlerConfig.enabled) {
    if (InterlockedCompareExchange(
            const_cast<LONG*>(&g_todAssetCrawlerState), 0, 0) != kTodAssetCrawlerIdle) {
      StopTodAssetCrawler("disabled");
    }
    InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerArmed), 0);
    return;
  }
  ...
  if (InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerArmed), 1, 0) == 0) {
    if (!BuildTodAssetCrawlerCatalog()) {
      InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerArmed), 0);
      return;
    }
    InterlockedExchange(
        const_cast<LONG*>(&g_todAssetCrawlerState),
        kTodAssetCrawlerLoadEntry);
  }

  const LONG state =
      InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCrawlerState), 0, 0);
  const LONG index =
      InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCrawlerEntryIndex), 0, 0);
  const LONG count =
      InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCrawlerEntryCount), 0, 0);
  if (state == kTodAssetCrawlerIdle || index >= count) {
    return;
  }

  if (state == kTodAssetCrawlerLoadEntry) {
    const TodAssetCrawlerEntry& entry = g_todAssetCrawlerEntries[index];
    WriteTodAssetCrawlerStatus(
        "loading index=%ld/%ld blockId=%ld name=%s",
        index + 1,
        count,
        entry.blockId,
        entry.name);
    if (!LoadTodAssetCrawlerEntry(scene, entry)) {
      WriteTodAssetCrawlerStatus(
          "load_failed_skipping index=%ld blockId=%ld name=%s",
          index + 1,
          entry.blockId,
          entry.name);
      const LONG nextIndex =
          InterlockedIncrement(const_cast<LONG*>(&g_todAssetCrawlerEntryIndex));
      if (nextIndex >= count) {
        StopTodAssetCrawler("completed_with_load_failures");
      }
      return;
    }
    InterlockedExchange(
        const_cast<LONG*>(&g_todAssetCrawlerFramesRemaining),
        g_todAssetCrawlerConfig.loadSettleFrames);
    InterlockedExchange(
        const_cast<LONG*>(&g_todAssetCrawlerState),
        kTodAssetCrawlerSettle);
    return;
  }

  if (state == kTodAssetCrawlerSettle) {
    if (InterlockedDecrement(
            const_cast<LONG*>(&g_todAssetCrawlerFramesRemaining)) > 0) {
      return;
    }
    InterlockedExchange(const_cast<LONG*>(&g_todAssetCaptureArmed), 0);
    InterlockedExchange(const_cast<LONG*>(&g_todAssetCrawlerProbeRequested), 1);
    InterlockedExchange(
        const_cast<LONG*>(&g_todAssetCrawlerState),
        kTodAssetCrawlerWaitForProbe);
    return;
  }

  if (state == kTodAssetCrawlerWaitForProbe) {
    const LONG captureArmed =
        InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCaptureArmed), 0, 0);
    const LONG captureState =
        InterlockedCompareExchange(const_cast<LONG*>(&g_todAssetCaptureState), 0, 0);
    if (captureArmed == 0 || captureState != kTodAssetCaptureIdle) {
      return;
    }

    const TodAssetCrawlerEntry& entry = g_todAssetCrawlerEntries[index];
    WriteTodAssetCrawlerStatus(
        "captured index=%ld/%ld blockId=%ld name=%s uniqueThisProbe=%ld",
        index + 1,
        count,
        entry.blockId,
        entry.name,
...
void RegisterTexture(
  ...
  for (LONG i = 0; i < count; ++i) {
    ...
    if (emptyIndex < 0 && g_textureInfo[i].texture == nullptr) {
      emptyIndex = i;
    }
  }
...
void UnregisterTexture(IDirect3DBaseTexture9* texture) {
  if (texture == nullptr) {
    return;
  }

  const LONG surfaceCount = ClampedCount(
      &g_surfaceInfoCount,
      static_cast<LONG>(sizeof(g_surfaceInfo) / sizeof(g_surfaceInfo[0])));
  for (LONG i = 0; i < surfaceCount; ++i) {
    if (g_surfaceInfo[i].ownerTexture == texture) {
      memset(&g_surfaceInfo[i], 0, sizeof(g_surfaceInfo[i]));
    }
  }

  IDirect3DBaseTexture9* replacement = nullptr;
  const LONG copyCount = ClampedCount(
      &g_textureCopyInfoCount,
      static_cast<LONG>(sizeof(g_textureCopyInfo) / sizeof(g_textureCopyInfo[0])));
  for (LONG i = 0; i < copyCount; ++i) {
    if (g_textureCopyInfo[i].source == texture) {
      replacement = g_textureCopyInfo[i].replacement;
      memset(&g_textureCopyInfo[i], 0, sizeof(g_textureCopyInfo[i]));
      break;
    }
  }

  const LONG textureCount = ClampedCount(
      &g_textureInfoCount,
      static_cast<LONG>(sizeof(g_textureInfo) / sizeof(g_textureInfo[0])));
  for (LONG i = 0; i < textureCount; ++i) {
    if (g_textureInfo[i].texture == texture) {
      memset(&g_textureInfo[i], 0, sizeof(g_textureInfo[i]));
      break;
    }
  }
...
ULONG APIENTRY Hook_TextureRelease(IDirect3DBaseTexture9* self) {
  if (g_origTextureRelease == nullptr) {
    return 0;
  }

  const ULONG references = g_origTextureRelease(self);
  if (references == 0) {
    UnregisterTexture(self);
  }
  return references;
}
...
void __fastcall Hook_TodSceneUpdate(void* self, void* edx) {
  RefreshTodAssetCrawlerConfig();
  if (BootstrapTodAssetCrawlerScene(self)) {
    return;
  }
  RunTodAssetCrawlerAtSceneUpdate(self);
  if (g_todAssetCrawlerConfig.enabled &&
      InterlockedCompareExchange(
          const_cast<LONG*>(&g_todAssetCrawlerBootstrapped), 0, 0) != 0) {
    return;
  }
  if (g_origTodSceneUpdate != nullptr) {
    g_origTodSceneUpdate(self, edx);
  }
}

void EnsureTodSceneUpdateHook() {
  if (InterlockedCompareExchange(
          const_cast<LONG*>(&g_todSceneUpdateHooked), 0, 0) != 0) {
    return;
  }

  void* scene = TodGlobalPointer(kTodCameraSystemGlobalRva);
  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t expectedUpdate = moduleBase + kTodSceneUpdateRva;
  if (scene == nullptr || moduleBase == 0 ||
      !IsCommittedMemoryRange(reinterpret_cast<uintptr_t>(scene), sizeof(void*), false)) {
    return;
  }

  void** vtable = *reinterpret_cast<void***>(scene);
  if (vtable == nullptr) {
    return;
  }
  for (DWORD index = 0; index < 64; ++index) {
    ...
    if (!IsCommittedMemoryRange(slotAddress, sizeof(void*), false)) {
      break;
    }
    if (reinterpret_cast<uintptr_t>(vtable[index]) != expectedUpdate) {
      continue;
    }
    if (HookVtableSlot(
            scene,
            index,
            Hook_TodSceneUpdate,
            &g_origTodSceneUpdate,
            "TOD Scene::Update")) {
      InterlockedExchange(const_cast<LONG*>(&g_todSceneUpdateHooked), 1);
      WriteTodAssetCrawlerStatus("scene_update_hook_installed vtableIndex=%lu", index);
    }
    return;
  }
}
...
void WriteRelativeJump5(BYTE* target, uintptr_t destination) {
  target[0] = 0xE9;
  const intptr_t displacement =
      static_cast<intptr_t>(destination) -
      (reinterpret_cast<intptr_t>(target) + 5);
  *reinterpret_cast<LONG*>(target + 1) = static_cast<LONG>(displacement);
}
...
bool InstallInlineJumpHook(
    ...
    SIZE_T patchBytes,
    const char* name) {
  if (target == 0 || hook == nullptr || original == nullptr || patchBytes < 5) {
  ...
  if (patchBytes == 5) {
    WriteRelativeJump5(targetBytes, reinterpret_cast<uintptr_t>(hook));
  } else {
...
void __fastcall Hook_TodSetTraverseDistance(
    void* self,
    void*,
    float distance) {
  // LOD experiment removed 2026-06-07: the shim no longer raises TOD's traverse/
  // activation distance. Pure passthrough so TOD uses its own stock values.
  if (g_origTodSetTraverseDistance != nullptr) {
    g_origTodSetTraverseDistance(self, distance);
  }
}

void InstallTodSetTraverseDistanceHook() {
  if (InterlockedCompareExchange(
          const_cast<LONG*>(&g_todSetTraverseDistanceHooked), 1, 0) != 0) {
    return;
  }

  const uintptr_t moduleBase = TodModuleBase();
  const uintptr_t target =
      moduleBase != 0 ? moduleBase + kTodSetTraverseDistanceRva : 0;
  if (target == 0 ||
      !IsCommittedMemoryRange(target, kTodSetTraverseDistancePatchBytes, false)) {
    InterlockedExchange(const_cast<LONG*>(&g_todSetTraverseDistanceHooked), 0);
    Log("TOD Node::SetTraverseDistance hook skipped: target unavailable");
    return;
  }

  const BYTE* targetBytes = reinterpret_cast<const BYTE*>(target);
  if (targetBytes[0] != 0x8B || targetBytes[1] != 0x49 ||
      targetBytes[2] != 0x30 || targetBytes[3] != 0x85 ||
      targetBytes[4] != 0xC9) {
    InterlockedExchange(const_cast<LONG*>(&g_todSetTraverseDistanceHooked), 0);
    Log(
        "TOD Node::SetTraverseDistance hook skipped: unexpected prologue "
        "%02X %02X %02X %02X %02X",
        targetBytes[0],
        targetBytes[1],
        targetBytes[2],
  ...
  if (!InstallInlineJumpHook(
          target,
          reinterpret_cast<void*>(Hook_TodSetTraverseDistance),
          &original,
          kTodSetTraverseDistancePatchBytes,
          "TOD Node::SetTraverseDistance")) {
    InterlockedExchange(const_cast<LONG*>(&g_todSetTraverseDistanceHooked), 0);
    return;
  }

  g_origTodSetTraverseDistance =
      reinterpret_cast<TodSetTraverseDistanceFn>(original);
}
...
DWORD TodTextureMaxAnisotropy() {
  DWORD value = g_todTextureQualityConfig.maxAnisotropy;
  if (value > g_deviceMaxAnisotropy) {
    value = g_deviceMaxAnisotropy;
  }
  return value > 0 ? value : 1;
}

DWORD OverrideTodSamplerState(
    DWORD sampler,
    D3DSAMPLERSTATETYPE type,
    DWORD value) {
  if (!g_todTextureQualityConfig.enabled ||
      sampler >= g_todTextureQualityConfig.samplerCount) {
    return value;
  }

  const DWORD maxAnisotropy = TodTextureMaxAnisotropy();
  if (type == D3DSAMP_MIPMAPLODBIAS) {
    return FloatToDword(g_todTextureQualityConfig.mipBias);
  }
  if (type == D3DSAMP_MAXANISOTROPY) {
    return maxAnisotropy;
  }
  if (type == D3DSAMP_MAXMIPLEVEL && g_todTextureQualityConfig.forceTopMip) {
    return 0;
  }
  if (type == D3DSAMP_MIPFILTER && g_todTextureQualityConfig.forceTopMip) {
    return D3DTEXF_NONE;
  }
...
HRESULT APIENTRY Hook_SetSamplerState(
    IDirect3DDevice9* self,
    DWORD sampler,
    D3DSAMPLERSTATETYPE type,
    DWORD value) {
  return g_origSetSamplerState(
      self,
      sampler,
      type,
      OverrideTodSamplerState(sampler, type, value));
}

void ApplyTodTextureQualityOverride(IDirect3DDevice9* device) {
  RefreshTodTextureQualityConfig();
  if (!g_todTextureQualityConfig.enabled ||
      device == nullptr ||
      g_origSetSamplerState == nullptr) {
    return;
  }

  const DWORD maxAnisotropy = TodTextureMaxAnisotropy();
  for (DWORD sampler = 0; sampler < g_todTextureQualityConfig.samplerCount; ++sampler) {
    g_origSetSamplerState(
        device,
        sampler,
        D3DSAMP_MIPMAPLODBIAS,
        FloatToDword(g_todTextureQualityConfig.mipBias));
    g_origSetSamplerState(
        device,
        sampler,
        D3DSAMP_MAXANISOTROPY,
        maxAnisotropy);
    if (g_todTextureQualityConfig.forceTopMip) {
      g_origSetSamplerState(device, sampler, D3DSAMP_MAXMIPLEVEL, 0);
      g_origSetSamplerState(device, sampler, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
    }
    if (maxAnisotropy > 1) {
      g_origSetSamplerState(
          device,
...
void InjectSunLight(IDirect3DDevice9* device) {
  ...
  // tunable from scripts/TODRemixShim.ini while we match TOD's sky sun.
...
void HookDevice(IDirect3DDevice9* device) {
  ...
  InstallTodSetTraverseDistanceHook();
  ...
  HookVtableSlot(
      device,
      kDeviceSetSamplerStateIndex,
      Hook_SetSamplerState,
      &g_origSetSamplerState,
  ...
  if (SUCCEEDED(device->GetDeviceCaps(&caps))) {
    g_deviceMaxAnisotropy = caps.MaxAnisotropy > 0 ? caps.MaxAnisotropy : 1;
  }
  ApplyTodTextureQualityOverride(device);
...
DWORD WINAPI WorkerThread(void*) {
  Log("TODRemixShim ASI loaded; gameRoot=%s log=%s", g_gameRoot, g_logPath);
