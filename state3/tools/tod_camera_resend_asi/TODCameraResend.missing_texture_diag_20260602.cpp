...
#define TOD_ENABLE_MISSING_TEXTURE_DIAG 1

namespace {

// Runtime support boundary, derived from TOD.exe:
// 1. cache/resend fixed-function view/projection before non-RHW indexed draws;
// 2. redirect TOD's texture-backed scene RTs to the primary path and skip the
//    stale fullscreen RT composites;
// 3. give Remix hashable DEFAULT-pool copies of TOD managed world textures.
// Texture classification, hash discovery, and content decisions stay out of the
// ASI and belong in Remix config or temporary offline tooling.
constexpr bool kEnableRuntimeLog = TOD_ENABLE_RUNTIME_LOG != 0;
constexpr bool kEnableMissingTextureDiag = TOD_ENABLE_MISSING_TEXTURE_DIAG != 0;
...
volatile LONG g_missingManagedTextureDraws = 0;
...
void InitializePaths(HMODULE module) {
  ...
  snprintf(
      g_diagLogPath,
      sizeof(g_diagLogPath),
      "%s\\rtx-remix\\logs\\tod-missing-texture-diag.log",
      g_gameRoot);
}
...
void DiagnosticLog(const char* fmt, ...) {
  ...
  EnsureLogDirectory();
  HANDLE file = CreateFileA(
      g_diagLogPath[0] != '\0' ? g_diagLogPath : "rtx-remix\\logs\\tod-missing-texture-diag.log",
      FILE_APPEND_DATA,
      FILE_SHARE_READ | FILE_SHARE_WRITE,
      nullptr,
      OPEN_ALWAYS,
...
void LogMissingManagedTextureCandidate(
    LONG drawCount,
    D3DPRIMITIVETYPE primitiveType,
    UINT primitiveCount,
    UINT vertexCount,
    const LayoutState& layout) {
  if (!kEnableMissingTextureDiag || layout.fvfRhw || layout.declarationPositionT ||
      layout.shaderActive || g_currentTexture0 == nullptr) {
    return;
  }

  const TextureInfo* texInfo = FindTextureInfo(g_currentTexture0);
  const bool managedRegular = IsManagedRegularTexture(texInfo);
  if (texInfo != nullptr && !managedRegular) {
    return;
  }

  const bool shouldCopy = ShouldCreateDefaultTextureCopy(texInfo);
  const bool likelyMissingHash = texInfo == nullptr || managedRegular;
  if (!likelyMissingHash) {
    return;
  }

  const LONG count = InterlockedIncrement(const_cast<LONG*>(&g_missingManagedTextureDraws));
  if (count > 160 && (count % 1000) != 0) {
    return;
  }

  const SurfaceInfo* rtInfo = FindSurfaceInfo(g_currentRenderTarget0);
  DiagnosticLog(
      "missing-texture-candidate #%ld: draw=%ld primType=%u primCount=%u vertices=%u "
      "fvf=0x%08lx decl=%p tex0=%p texKnown=%d texInfo=%ux%u/%s usage=0x%08lx "
      "pool=%u levels=%u managed=%d shouldCopy=%d compressed=%d whitelistedA8=%d "
      "rt0=%p rtInfo=%ux%u/%s rtFromTex=%d",
      count,
      drawCount,
      static_cast<unsigned>(primitiveType),
      primitiveCount,
      ...
      managedRegular ? 1 : 0,
      shouldCopy ? 1 : 0,
      texInfo != nullptr && IsCompressedTextureFormat(texInfo->format) ? 1 : 0,
      IsWhitelistedUncompressedCopyCandidate(texInfo) ? 1 : 0,
      g_currentRenderTarget0,
      rtInfo != nullptr ? rtInfo->width : 0,
      rtInfo != nullptr ? rtInfo->height : 0,
      rtInfo != nullptr ? FormatName(rtInfo->format) : "unknown",
...
HRESULT APIENTRY Hook_DrawIndexedPrimitive(
  ...
  LogMissingManagedTextureCandidate(
      count,
      primitiveType,
      primitiveCount,
      numVertices,
