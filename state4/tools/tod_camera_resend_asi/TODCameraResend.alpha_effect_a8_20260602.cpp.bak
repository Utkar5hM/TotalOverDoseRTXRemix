...
constexpr bool kBindAlphaEffectA8Copies = true;
...
volatile LONG g_alphaEffectA8CopyBinds = 0;
...
bool IsAlphaEffectA8Draw(const LayoutState& layout) {
  if (!kBindAlphaEffectA8Copies) {
    return false;
  }
  ...
  if (depthEnabled && depthWriteEnabled) {
    return false;
  }
...
IDirect3DBaseTexture9* BindPreTransformedA8CopyForDraw(
  ...
  const bool alphaEffect = IsAlphaEffectA8Draw(layout);
  if ((!preTransformedUi && !alphaEffect) || g_origSetTexture == nullptr) {
    return nullptr;
  }
  ...
  if (SUCCEEDED(result)) {
    if (alphaEffect) {
      InterlockedIncrement(const_cast<LONG*>(&g_alphaEffectA8CopyBinds));
    } else {
